...
- **ACTUAL:** `market_radar.scan_sector()` in `market_radar.py:321` runs for
  ...
  **Unified lifecycle gate (2026-07-01):** `_candidate_is_live()` suppresses TRADE THIS
  on two conditions: (1) favorable drift — price ≥75% of entry→target distance (entry
  window closed); (2) adverse drift — price ≥75% of entry→stop distance (setup
  invalidated by market). Single function, both directions, one owner. Commit `2328dac`.
  4H/1H panels now have COPY TRIGGERS and OPEN COCKPIT matching 15M parity. Same commit.
...
| `ledger_closing_engine.py` | Every 60 sec | `CampaignLog` (APPROVED + 4H/1H CANDIDATES, unclosed) + MEXC live price + Kraken 1m OHLCV (Phases 2+4) | `CampaignLog` (status, realized_pnl, entry_filled_at, t2_reached, t3_reached, max_target_reached, closed_at); `session_audit_log` (`backfill_outcome` after APPROVED close — non-blocking). **Four-phase engine** (W-9 2026-06-11, Phase 4 2026-06-30): Phase 1 watches APPROVED+is_canonical for entry fill → EXPIRED at session_expires_at; Phase 2 monitors APPROVED+is_canonical+filled via Kraken 1m OHLC; Phase 3 observes T2/T3 post-T1; Phase 4 (NEW 2026-06-30) monitors 4H_CANDIDATE+1H_CANDIDATE via same OHLC scan, closes on STOP/T1/expiry. Candidates enter with entry_filled_at=detection_time, session_expires_at=now+5d(4H)/now+2d(1H) set at write time in gravity_engine. |
...
| `signal_accuracy_tracker.py` | Every 4h | `JewelSnapshotLog`, `DecisionJournal`, `CampaignLog` (4H/1H canonical), `SessionAuditLog` | `SignalAccuracyLog` — two-pass: captures predictions immediately (outcome fields NULL), checks outcome only once a signal has aged past `lookahead_hours` (4h) |
| `signal_flagging_engine.py` | Every 6h | `SignalAccuracyLog` (signals with N≥100 resolved outcomes) | `SystemAlertLog` (WARN/CRITICAL alerts for signals below the accuracy threshold — FLAG_ONLY, no weight changes), `SignalHealthLog` (per-tick health snapshot) |
| `accuracy_report_generator.py` | Weekly | `SignalAccuracyLog`, `SystemAlertLog` | `AccuracyReport` (top/bottom performers, trending up/down, flagged-signals summary) |

**Phase 2 — Signal Accuracy & Flagging (FLAG_ONLY, deployed 2026-07-16, commit `d485e34`).** `signal_weight_manager.py` stores accuracy-derived weights (`SignalWeight` table) for informational/manual use only — `get_weight()`/`get_all_weights()` for read access, `quarantine_signal()` for a human to manually zero a signal's weight. Not wired into `krown_system.py` or `mtf_confluence_scanner.py`'s confluence scoring; no automatic weight adjustment exists in this phase. Config: `config/phase2_config.json` (`action_mode: "FLAG_ONLY"`, `min_samples: 100`, `affect_trading: false`). Dashboard: Errors tab shows active alerts (with resolve action), Analysis tab shows the latest weekly report.
...
| 2026-07-01 | 1B, gravity_engine, market_radar.html | **Single-target 4H/1H + unified lifecycle check + radar parity.** (1) `gravity_engine.py`: removed T2/T3 from `_detect_4h_bos()` and `_detect_1h_bos()`. Decision: Crown's strategy code uses one structural target per trade (S2/S3 exit at single structural high/low); Kabroda's own reasoning lands the same — T1 (equal-leg measured move from broken zone) is the only structurally anchored target; T2/T3 were unanchored equal-leg extrapolations. CampaignLog `t2`/`t3` now NULL for 4H/1H candidates — ledger engine already guards `if c.t2 is not None`. (2) `market_radar.py`: replaced `_is_bos_stale()` (favorable drift only) with `_candidate_is_live()` — single unified function checking both favorable drift (price ≥75% entry→target) and adverse drift (price ≥75% entry→stop). Root cause: a candidate that moved 68.8% toward its stop still showed TRADE THIS — the old function was blind to adverse movement. (3) `templates/market_radar.html`: 4H/1H panels now have COPY TRIGGERS and OPEN COCKPIT buttons matching 15M parity. `window.tfCandidateMemory` seeded on every TF stack render. `openTfCockpit()` and `copyTfLevel()` functions added. CSS `.tf-btn-copy`, `.tf-btn-cockpit` added. | owner + Claude Code | Root cause (1): T2/T3 were built as mathematical extrapolations with no structural anchor in `gravity_memory`; Crown's material confirmed single structural exit. Root cause (2): `_is_bos_stale()` was direction-specific and only checked favorable drift — adverse drift went unchecked. Root cause (3): 4H/1H panels were display-only by early build-sequence design; owner needed full parity once candidates went live. | `gravity_engine.py`: removed 6 lines per function (t2/t3 vars + computation + CampaignLog write + print). `market_radar.py`: replaced 18-line `_is_bos_stale()` with 36-line `_candidate_is_live()`, updated `_which_tf_today()` calls. `market_radar.html`: +~60 lines (CSS, seed logic, two functions, tfRow buttons). Commit `2328dac`. |
| 2026-07-01 | database.py, gravity_engine | **Version-tag fix: v2/v3 discriminator.** Candidate 112 had T2/T3 populated under the pre-cutover staged-target logic, tagged `v2`. New single-target candidates were about to inherit the same tag with a different row shape (T2/T3 always NULL), risking silent pooling of two incompatible trade-construction shapes in any future audit query. Bumped `target_logic_version` to `'v3'` for all candidates written under single-target logic (both 4H and 1H detectors). `v2` is now frozen as the legacy staged shape; `v3` is the current single-target shape. Documented permanently in `database.py`'s `CampaignLog` comment block. **Deploy confirmed live** via Render boot log for commit `6fae865`: `GRAVITY ENGINE: Initializing background loop (v3 target logic, STRICT SSOT MODE)...` — the literal changed print line, proving the exact commit is running, not a stale worker. No traceback, `Your service is live`. | owner + Claude Code | No other Python file filters on `target_logic_version`, confirmed via repo-wide grep before the tag bump — no downstream consumer breaks. | `database.py`: `target_logic_version` comment block rewritten with explicit v1/v2/v3 discriminator. `gravity_engine.py`: two `CampaignLog` writes + two print statements + boot print + two section headers updated v2→v3. Commit `6fae865`. |
| 2026-07-01 | 1B, market_radar.html, notify.py (new), gravity_engine, ledger_closing_engine | **Resolved-candidate display fix + admin email notifications (4H/1H open/close).** Root cause: candidate 112 (1H LONG) closed CLOSED_WIN at 04:45 UTC — a real +1R win — but the radar rendered it as live for hours with COPY/COCKPIT still active, owner found out by accident. Fix 1 (`market_radar.py`, `templates/market_radar.html`): `_get_tf_system_verdicts()` set `status:"BOS_ACTIVE"` unconditionally whenever a 4H/1H row existed for today — never checked `closed_at`. Same engine-vs-body pattern as the lifecycle fix: Phase 4 already knew the correct answer (candidate closed, correctly recorded), the display never read it. New helper `_tf_candidate_verdict(c)` returns `status:"RESOLVED"` (+ `outcome`, `realized_pnl`) once `closed_at` is populated, else `"BOS_ACTIVE"`. `_which_tf_today()` confirmed to already exclude RESOLVED from TRADE THIS via the `=="BOS_ACTIVE"` check — explicit comment added documenting both suppression paths (price-drift + resolved-state) so a future edit can't silently break either. `tfRow()` in the template renders a third state: RESOLVED shows the outcome plainly ("RESOLVED — WIN +1.00R" / "LOSS −1.00R" / "EXPIRED (...)"), disables COPY/COCKPIT ("NO ACTION AVAILABLE"). `tfCandidateMemory` seeding now deletes the entry for any non-BOS_ACTIVE candidate. Fix 2 (`notify.py` new module, wired into `gravity_engine.py` + `ledger_closing_engine.py`): `send_admin_email()` via stdlib `smtplib`, recipient `SMTP_DEST` — already provisioned in Render (`SMTP_USER`/`SMTP_PASS`/`SMTP_DEST` existed unused, confirmed via repo-wide grep before building) — used directly instead of a new admin-query system. Non-blocking, logs and returns False on any failure, never raises. Trigger 1 (open): both `_detect_4h_bos()`/`_detect_1h_bos()`, immediately after `CampaignLog` commit — symbol/timeframe/bias/entry/stop/target/`target_logic_version`. Trigger 2 (close): new `_notify_candidate_closed(c)` helper, called at all four Phase 4 resolution branches (CLOSED_WIN, CLOSED_LOSS, CLOSED_AT_EXPIRY, EXPIRED-no-data) — outcome, realized PnL, time-to-resolve. No new rate-limiting — daily dedup already caps this to ≤2 emails/timeframe/day. | owner + Claude Code | Root cause (1): display query assumed "row exists for today" == "still live," never read the engine's own resolution state. Root cause (2): SMTP credentials existed in Render but were never wired to any code path — confirmed via grep before deciding to use them rather than standing up a new provider. | `market_radar.py`: +`_tf_candidate_verdict()` helper, `_get_tf_system_verdicts()` rewritten to call it, comment added to `_which_tf_today()`. `templates/market_radar.html`: `tfRow()` +RESOLVED branch (~25 lines), memory-seed guard updated. `notify.py`: new file, `send_admin_email()`. `gravity_engine.py`: +import, +2 notify call sites (one per detector). `ledger_closing_engine.py`: +import, +`_notify_candidate_closed()` helper, +4 call sites (one per Phase 4 branch). Commit `fd9e723`. Also added `POST /api/admin/test-notify` (`main.py`, commit `6cfa2bb`) — admin-gated endpoint that fires a test email using the real Render-resolved SMTP config, so delivery can be verified from the live site without any credential re-entry. **Deploy confirmed live for both commits (clean boot logs, no traceback). Real SMTP delivery confirmed end to end** — owner ran the test endpoint via browser console (`{ok: true, smtp_host: 'smtp.gmail.com', ...}`) and confirmed the email actually arrived at `SMTP_DEST` (pasted back the literal body text). Still open: no real 4H/1H candidate has opened/closed since deploy, so the actual trigger-fired emails (as opposed to the manual test) haven't been observed yet — same code path, just not yet exercised by a live event. |
...
| 2026-07-03 | database.py | **CRITICAL FIX — t2/t3 NOT NULL constraint blocked every 4H/1H candidate write since the v3 single-target deploy.** Owner pasted a live production error: 1H SHORT BOS on BTC/USDT hit `psycopg.errors.NotNullViolation` on column `t2`, INSERT rolled back. Root cause: the 2026-07-01 single-target refactor (commit `2328dac`) intentionally writes `t2=None, t3=None` for 4H/1H candidates, but `campaign_logs.t2`/`t3` were still `NOT NULL` at the DB level — never updated when the code changed. **Every 4H/1H candidate write failed silently from the v3 deploy (~2026-07-01 15:08 UTC) until this fix — roughly a day and a half of zero recorded candidates.** Not a capital-risk issue (CANDIDATE-only rows, no live capital; 15M system unaffected, still always populates t2/t3) but real audit data was lost for that window and is unrecoverable (rows rolled back, id sequence just skipped ahead). Fix: `ALTER TABLE campaign_logs ALTER COLUMN t2/t3 DROP NOT NULL` added to the existing `init_db()` ALTER-TABLE-in-try/except pattern; ORM model `nullable=True` with explanatory comment. Verified safe: `ledger_closing_engine.py` already null-guards, `kabroda_mas_flow.py`/`publisher_crew.py` only touch the unaffected 15M system, `main.py` passthrough serializes `None` to JSON `null` safely. | owner + Claude Code | Root cause: the single-target refactor changed what the code writes but nobody checked whether the underlying DB column allowed the new value. Standing lesson: when removing a previously-required field from a constructor call, grep the actual `Column(...)` definition, not just the write site. | `database.py`: 2-column ALTER TABLE block added to `init_db()`, ORM model `t2`/`t3` changed `nullable=False`→`nullable=True`. Commit `e1b9f7e`. |
| 2026-07-06 | 3B, database.py | **Runner mechanic — shadow-mode partial-profit trailing, 15M only, record-only.** Ten new nullable `CampaignLog` columns (`shadow_runner_active`/`_stop`/`_ema21`/`_bucket_ts`/`_bucket_close`/`_last_scan_ts`/`_closed_at`/`_exit_reason`/`_leg2_r`/`_blended_r`). Phase 2's existing T1-hit branch gains 3 lines seeding `shadow_runner_active=True`, `shadow_runner_stop=entry_price` (breakeven) — real `status`/`realized_pnl`/`closed_at` completely unchanged. New Phase 3B walks 1m candles (reuses `_fetch_1m_since()` as-is, no new fetch) chronologically, resamples into 15m buckets on the fly, updates a running EMA21 incrementally, ratchets the shadow stop only favorably, resolves on stop touch / T3 touch / 5-day time cap. Design caught its own look-ahead-bias risk before shipping: naively recomputing "current EMA21" per tick and applying it retroactively across a re-scanned window would test old candles against a stop level the EMA hadn't reached yet historically — fixed by only ratcheting with bars closed strictly before the candle under test, mirroring how Phase 2 already scans in order. Phase 4 (4H/1H) explicitly untouched — no EMA21 infra there, and 4H/1H is still record-only/unvalidated. `total_contracts` confirmed still hard-coded `0.0` everywhere (never read) — "close 50%" modeled as a blended-R calc (`0.5×realized_pnl + 0.5×shadow_runner_leg2_r`), not a real partial fill, since this engine is monitoring/audit-only, not live order execution. | owner + Claude Code | Master plan Component 4 (Runner Mechanic), re-scoped: original gate ("4H system live with N>0") dropped in favor of targeting the proven 15M system directly, in shadow mode first so real recorded outcomes are unaffected until the owner reviews real shadow-vs-actual comparisons. | `ledger_closing_engine.py`: Phase 2 T1-branch +3 lines, +`_floor_to_15m()`/`_update_ema21()` helpers, +Phase 3B (~65 lines), header/boot-log strings updated. `database.py`: +10 columns + migration block. |
| 2026-07-07 | 3B | **Runner mechanic, part 2 — 4H/1H structural-zone trailing, shadow mode, record-only.** Extends the shadow runner to 4H/1H `CANDIDATE` rows using `gravity_memory` supply/demand zones instead of an EMA, per the original master plan's own Component 4 spec (4H/1H moves too slowly for a 15m-style EMA trail). Zero new `database.py` columns — the ten `shadow_runner_*` columns from the 15M version are generic and mutually-exclusively populated (Phase 3B filters `session_timeframe=="15M"`, this new Phase 4B filters `mas_approval_status IN (4H_CANDIDATE,1H_CANDIDATE)`). Phase 4's shared T1-hit branch gains the same 3-line seed as Phase 2's. New `_nearest_zone_by_price()` helper — a genuine price-proximity query against `GravityMemory` (patterned after `trade_structure_analyst.py`'s `_find_wall_long()`/`_find_wall_short()` shape), deliberately not reusing `_nearest_pivot_in_window()` since that helper's recency-first ordering is correct for stop *selection* at construction (item #1) but wrong for trailing. Price filter enforces "ratchet only favorably" directly in SQL; `timestamp <= candle_ts` enforces the same no-look-ahead discipline the 15M EMA trail required. T3 stays the fixed v4 Fibonacci target — only the stop trails. Time cap 5d(4H)/2d(1H) from the real T1 close, reusing the existing per-timeframe duration constants. | owner + Claude Code | Owner watched a real 4H candidate fire (hand-verified its T1/T2/T3 against the v4 formula — all three matched exactly) and asked why 4H/1H wasn't being tracked too, since "we want to track everything." The original 15M-only scoping's stated reason ("not real money yet") didn't actually hold since shadow tracking is observational on both systems — the real reason was no EMA21 infra for 4H/1H, which a proper zone-based design solves without needing one. | `ledger_closing_engine.py`: `GravityMemory` import added, Phase 4 T1-branch +3 lines, +`_nearest_zone_by_price()` helper, +Phase 4B (~70 lines), header strings updated. `database.py`: no changes. |
| 2026-07-07 | NEW MODULE | **KULTI Long-Term Investing module — brand-new, separate, advisory-only.** Implements Eric Crown's KULTI course (confluence-count BTC framework) plus two Kabroda-native additions (AI interpreter layer, gravity-wall cross-confirmation) and one honestly-sourced addition Crown's own course doesn't document (real Capriole Hash Ribbons, not a placeholder). Two new tables (`LtiCheckpoint` append-only, `LtiProtocol` mutable settings row, both via `Base.metadata.create_all()`, no ALTER TABLE). Three new modules: `hash_ribbons.py` (blockchain.com Charts API, stdlib `urllib.request`, no new dependency), `lti_engine.py` (native weekly Kraken OHLCV — not resampled daily, too shallow — + 9 components reusing `battlebox_pipeline`'s existing BBWP/PMARP/RSI/EMA functions + Crown's confluence-count engine + gravity cross-confirmation), `lti_interpreter.py` + `agents/lti_interpreter.md` (new Bucket-B interpreter, mirrors `gravity_interpreter.py`'s exact shape, logs to existing `InterpreterLog`). `main.py`: new `_seconds_until_month_start()` + `run_monthly_lti_scheduler()` (mirrors `run_weekly_scheduler()`), `GET /suite/lti` + `POST /api/lti/protocol`, registered in `lifespan()`. New `templates/lti.html` + nav link, built against the site's real design-token system (`site.css`'s `:root` variables), not invented colors. Every component live-tested against real data before wiring together: Hash Ribbons returned a real CAPITULATION reading on first call; the engine's full pipeline ran against real Kraken weekly BTC data and the confluence math was hand-verified; the interpreter correctly fail-opened with no local API key; `main.py` imports cleanly; the template test-rendered in both empty and populated states. | owner + Claude Code | Owner's original request ("look into whether we can build KULTI") plus an explicit follow-up decision to include the AI/gravity/Hash-Ribbons improvements in the first build rather than defer them — see WORK_LOG.md's full KULTI entry for the course-fidelity research and the honesty discipline applied to undocumented Crown components. | `database.py`: +2 models. `hash_ribbons.py`, `lti_engine.py`, `lti_interpreter.py`, `agents/lti_interpreter.md`, `templates/lti.html` all new. `main.py`: +2 imports, +1 helper, +1 scheduler, +2 routes, +1 `lifespan()` registration. `templates/nav.html`: +1 link. |
| 2026-07-14 | market_radar.py, market_radar.html | **TradingView "Mission Plotter" HUD key fixed — real 9-field contract confirmed from the actual Pine source, three separate builders were each broken.** Owner reported the indicator showing "DATA MISSING" when copying a 4H/1H candidate; pasted the real Pine code, which requires exactly `bias\|status\|entry\|stop\|tp1\|tp2\|tp3\|macro\|micro` (9 fields, `array.size(parts) >= 9` gate). Found three independently-broken key builders: 4H/1H `copyTfLevel()` (5 fields, non-numeric text), 4H/1H `openTfCockpit()` (7 fields), and even the 15M `renderSnapshotGrid()` "COPY TO HUD" key (7 fields, no macro/micro) — the last one matches a 2026-06-19 WORK_LOG note that flagged this exact risk and was never confirmed until now. `market_radar.py`'s `_tf_candidate_verdict()` was also silently discarding `t2`/`t3`/`macro_bias`/`dominant_direction` for 4H/1H even though all four already exist on `CampaignLog`. Fixed: backend now surfaces all four fields; new shared `buildMissionKey9()` JS helper replaces all three ad hoc string builders with real `NEUTRAL`/`WAITING` fallbacks. 15M's key reuses the already-cached `mtf_cached.confluence_direction` for macro/micro (poll-driven, must stay DB-only); 4H/1H uses the real per-candidate `macro_bias`/`dominant_direction` directly — no new live fetch added anywhere. | owner + Claude Code | Owner-reported bug, diagnosed exactly once the real Pine Script source was shared rather than guessed from a screenshot. | `market_radar.py`: `_tf_candidate_verdict()` +4 fields, default dict updated. `templates/market_radar.html`: new `buildMissionKey9()`, `openTfCockpit()`/`copyTfLevel()`/`renderSnapshotGrid()` key builders replaced, `tfCandidateMemory` seeding carries the new fields. |
| 2026-07-09 | main.py, suite_dashboard.html, confluence.html | **Executive Dashboard audit — two real bugs fixed, one metric clarified, Confluence page gets a plain-English summary.** Owner-requested audit of `/suite/dashboard` found: (1) "Net R Lifetime" KPI + Cumulative Performance chart computed R as a win/loss COUNT (`wins - losses`, hardcoded +-1.0), ignoring the real fractional `realized_pnl` the 2026-07-04/05 `_frac_r()` fix already produces — fixed to sum real `realized_pnl` (`CLOSED_WIN`/`CLOSED_LOSS`/`CLOSED_AT_EXPIRY`). (2) "4H Outcome vs. Session Bias" chart was reading `DecisionJournal.kinematic_grade` (old 15M radar-scan field) under a "4H" label — rewired to real `CampaignLog` 4H/1H `kinematic_grade` data, N shown in each bar label. (3) "0% Cache Hit Rate" investigated and confirmed correct, not a bug — Anthropic's 5-min ephemeral cache TTL structurally can't hit given most agents fire once per session; added an explanatory tooltip, left caching mechanism untouched (commlink chat + same-run retries are the real, occasional beneficiaries). (4) New deterministic "What This Means" plain-English summary card on `/suite/confluence`, plus fixed a real labeling bug found in the process (bare divergence flag chip read as contradicting the bias chip; now suffixed "DIVERGENCE"). | owner + Claude Code | Owner asked directly whether the dashboard had been audited and whether Confluence needed a plain-English section — both answered by reading the actual route code, not guessing from screenshots. | `main.py`: `/api/dashboard/overview`, `/api/dashboard/accuracy`, `/api/dashboard/mas-history` — real realized_pnl math, real 4H/1H query. `templates/suite_dashboard.html`: trade-table color logic, chart subtitle/labels, cache KPI tooltip. `templates/confluence.html`: new `buildPlainRead()` + card, divergence flag label fix. |
| 2026-07-09 | NEW ROUTE, gravity_engine, audit_ai.py | **MTF confluence exposed as its own view + fed into Component 6 as H10.** Researched before scoping (same discipline as Component 6's harness/ discovery) — `mtf_confluence_scanner.run_mtf_confluence_scan()` already computes a live, session-lock-independent 5-TF (15M/1H/4H/1D/1W) direction vote with its own aligned/opposing summary; it just wasn't surfaced standalone (only reachable via Market Radar's bundled scan). Part A: new `GET /api/confluence` route calls the scanner directly and additionally surfaces the macro engine's independent weekly-200-SMA trend read (`battlebox_pipeline._fetch_weekly_200sma()`) alongside the scanner's own EMA-vote read, labeled side by side rather than silently picked — new `templates/confluence.html` + `GET /suite/confluence` + nav link. Part B: `gravity_engine.py`'s two detectors call the scanner once per loop tick (shared, not doubled) and stash `dominant_direction`/`confluence_score` onto `CampaignLog` at candidate-creation time (two new nullable columns, same migration pattern as `macro_bias`/`kinematic_grade`); new `H10_TF_AGREEMENT` in `audit_ai.py` reuses `harness.tier_labels`/`harness.binomial_checkpoint` exactly like H7-H9 — no new statistical infrastructure. Record-only, does not gate candidate creation. Part C (economic calendar/FOMC tracking) explicitly scoped but not built — genuinely new territory, needs its own future data-source research pass. | owner + Claude Code | Owner's repeated "track everything, multi-timeframe, poker-player-watching-the-whole-table" request — most of what was about to be scoped as new work turned out to already exist, just unexposed. | `main.py`: +1 route (`GET /api/confluence`), +1 page route (`GET /suite/confluence`). `templates/confluence.html` new. `templates/nav.html`: +1 link. `database.py`: +2 nullable `CampaignLog` columns (`dominant_direction`, `confluence_score`) + migration block. `gravity_engine.py`: +`mtf_confluence_scanner` import, both detector signatures +`confluence` param, +confluence scan call in `run_gravity_ingestion_loop()`. `audit_ai.py`: +`_check_tf_agreement()` (H10_TF_AGREEMENT), digest "why" block extended with confluence fields. |
| 2026-07-08 | NEW MODULE, harness/ | **Component 6 — Daily System Audit, integrated with the pre-existing `harness/` ecosystem (not duplicated).** Mid-build, a duplicate-table crash on `Base.metadata.create_all()` caught that a full statistical audit ecosystem already existed (`harness/query_layer.py`, `join_logic.py`, `tier_labels.py`, `binomial_checkpoint.py`, `baseline.py`, `audit_runner.py` — H1-H6 against `SessionAuditLog`, real binomial significance testing, writes to the real `AuditSuggestionLog`) but had never been wired to run automatically anywhere — confirmed via full `lifespan()` review, every harness module was manual-invocation-only. Fix, three parts: (1) folded `harness.audit_runner.main()` into the existing `run_weekly_scheduler()` as a third dedup-checked task, kept at weekly cadence deliberately (preserves the `consecutive_runs_surfaced` "3 consecutive weeks" escalation semantics `audit_runner.py` was designed around — daily would cheapen it to 3 days). (2) New `audit_ai.py`: three hypotheses in the confirmed-untouched 4H/1H space (`kinematic_grade`/`energy_grade` correlation, 4H macro-bias alignment, runner-mechanic shadow-vs-real), daily, reusing `harness.tier_labels.tier_label()` and `harness.binomial_checkpoint.run_checkpoint()` rather than reimplementing statistics, writing to the existing `AuditSuggestionLog` under new ids `H7_KINEMATIC_4H1H`/`H8_MACROBIAS_4H`/`H9_RUNNER`. (3) New `DailyAuditLog` table + `build_daily_digest()`: per-trade "why," all three timeframes (15M's `mas_executive_brief`/`structure_reasoning`; 4H/1H's `macro_bias`/`kinematic_grade`/`energy_grade`/`htf_anchor_type`/`htf_anchor_price`) — pure fact-surfacing, no equivalent existed anywhere in `harness/`. New daily scheduler (23:45 UTC) runs (2)+(3). `/admin/export-audit-ledger` extended with `start_date`/`end_date`, response now includes `daily_digests`/`audit_suggestions`/`trials`. New admin.html card (digest summary, findings table, date-range COPY JSON button). | owner + Claude Code | Real 4H/1H losing stretch (candidates 143/141/140/138/137/136, 4-for-4 losses, all TANGLED/sub-MODERATE-energy) prompted the owner's "log, track, audit, always be looking, like a poker player" directive — Component 6 was already spec'd in the master plan (2026-06-29) but never built. | `database.py`: +1 model (`DailyAuditLog` only — `AuditSuggestionLog` imported from the pre-existing definition, not redefined). `audit_ai.py` new. `main.py`: audit_runner folded into weekly scheduler, +1 new daily scheduler, +1 `lifespan()` registration, export route extended, admin route passes new context. `templates/admin.html`: new card. |
