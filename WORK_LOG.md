...
## ► NEXT SESSION START
*End-of-session marker: 2026-07-16*

**Phase 2 (Signal Flagging) deployed — FLAG_ONLY, N≥100, no live trading impact (2026-07-16, commit `d485e34`).** Built on top of Phase 1's Signal Accuracy Tracker: `accuracy_decay_tracker.py` computes per-signal accuracy trend/health, `signal_flagging_engine.py` flags underperformers into `SystemAlertLog` every 6h, `accuracy_report_generator.py` writes a weekly `AccuracyReport`. Reviewed before commit and substantially reworked — the original version pitched a simultaneous AUTO-vs-FLAG_ONLY A/B experiment, but `action_mode` was a single global config value with no per-signal assignment, so the `FLAG_ONLY` arm could never populate; that framing was dropped in favor of an honest flagged-signals summary. `min_samples` raised from 20 to 100 and `action_mode` switched from `AUTO` to `FLAG_ONLY` to match this project's own N≥100/3-consecutive-weekly-confirmations discipline (established earlier this session via the audit-AI tiers and the v4/v5 stop-symmetry backtest rejection). All auto-weight-adjustment code removed — this phase only creates alerts for human review, no automatic changes. `get_accuracy_adjusted_confidence()` (documented as "used by the pipeline to adjust signal confidence," with `affect_trading: true` already set) was deleted outright rather than left disabled by config, since nothing called it yet but it was designed to be wired into `krown_system.py`/`mtf_confluence_scanner.py` confluence scoring with no further review checkpoint. Also fixed: the flagging engine originally skipped both alert creation *and* weight updates for any signal with an existing unresolved alert, so a signal only ever got evaluated once — now the existing alert refreshes with current accuracy/severity data every 6h tick. Caught in final review: `config/__init__.py`'s `_DEFAULT_CONFIG` fallback still had the old `AUTO`/20/`true` values even after `signal_flagging_engine.py`'s own local fallback was corrected — fixed to match. First tick post-deploy: 25 health records, 0 alerts (expected — no signal has reached N≥100 resolved outcomes yet). Next: let it run ~2 weeks accumulating real data, then design Phase 3 from what the data actually shows.

**ROOT CAUSE FOUND for the blank M2 dashboard tabs — a single missing `</div>` (2026-07-16).** The `min-height`/forced-reflow commit below was a defensive guess that couldn't have worked, because the real bug wasn't in the tab's own CSS at all. Went back to first principles and read `suite_dashboard.html` linearly instead of grepping for specific properties, and counted `<div>`/`</div>` balance across the Overview tab section: 51 opens vs. 49 closes. Traced it precisely — the "Trade History" `<div class="table-section">` (opened line 212) was missing its closing `</div>`; the next `</div>`, labeled `<!-- end tab-overview -->`, closed the trade-history section instead of `#tab-overview` itself, leaving `#tab-overview` permanently unclosed in the parsed DOM. Every tab after it (`#tab-live-system`, `#tab-parameters`, `#tab-errors`, `#tab-analysis`) silently became a **child** of `#tab-overview` instead of a sibling. The moment a user clicked any other tab, the JS's `document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'))` stripped `.active` from `#tab-overview` too (it's a `.tab-content` element) — dropping it to `display:none` — and since a `display:none` ancestor hides its entire subtree regardless of the descendant's own `display` value, the newly-active child tab collapsed to zero size even though its own computed style correctly said `display:block`. This is exactly why every diagnostic pointed two different directions at once: `getComputedStyle` on the child was correct (CSS cascade doesn't see ancestor visibility), while `getBoundingClientRect()` was genuinely `{0,0,0,0}` (real ancestor-driven collapse) — both true simultaneously, which is what made this so hard to explain from the child element alone. Fixed by adding the missing `</div>`; full-file div balance now 121 open / 121 closed (previously 119/121... effectively 2 short across the whole file, all in this one spot). 83/83 tests still pass. The earlier `min-height`/reflow workaround (commit `d284920`) is harmless and left in place as defense-in-depth, but was never the actual fix.

**M2 dashboard tabs render blank in production despite everything checking out correct — defensive fix shipped, root cause not found (2026-07-16).** Post-`0bf6dd8` deploy, Live System/Parameters/Errors/Analysis tabs still showed nothing on screen. Extensive live debugging session with the owner across Chrome, Edge, and incognito profiles (ruled out caching, extensions, session state): confirmed via DevTools that all 4 API calls return 200 with real payloads, the JS correctly populates real DOM content (40+ rows verified in `liveSessionsBody`), computed CSS is correct (`display: block`, correct text color, normal box model on individual cells), no duplicate IDs, no conflicting stylesheet rules, no animations/`content-visibility`/`contain` properties involved — and yet `document.getElementById('tab-live-system').getBoundingClientRect()` returns `{x:0,y:0,width:0,height:0}` on the live page, confirmed with the tab genuinely active. `elementFromPoint()` at that screen location returns bare `<html>`, consistent with a real zero-height collapse, not something painted over the content. Could not trace this to a root cause after exhausting CSS/JS static analysis and live inspection. Shipped a defensive (not diagnosed) fix: `min-height: 400px` + `width: 100%` on `.tab-content.active`, plus a forced synchronous reflow (`void panel.offsetHeight`) in the tab-click handler. 83/83 tests still pass; needs live verification on next deploy. If this doesn't resolve it, the next debugging step should be inspecting actual paint/compositing layers (Chrome's "Layers" DevTools panel) or testing on a completely different machine, since every DOM/CSS-level explanation has been ruled out.
...
1. **`timeframe_analyzer.py`'s `_fetch_campaigns()` had the exact `is_canonical` bug already fixed once this week in `audit_ai.py` (2026-07-09).** `WHERE is_canonical = TRUE` silently excludes every 4H/1H candidate row (deliberately `False` by design, not a real/fake data flag — see the 2026-07-09 entry below). Since the module's entire stated purpose is comparing 15M/1H/4H performance, this would have permanently reported `NO_DATA` for 2 of the 3 timeframes. Same fix pattern as `_real_btc_row()`: `(is_canonical = TRUE OR session_timeframe IN ('4H','1H')) AND symbol = 'BTC/USDT'`.
2. **`market_context_oracle.py`'s own claimed fix had a real bug in it.** The v1.1 "fix" added a flat-DataFrame fallback branch for when yfinance's MultiIndex collapses (a real, plausible edge case if some tickers in the SPX/DXY/VIX batch fail), but that branch didn't check *which* ticker the flat data belonged to — it would have silently assigned the same price/trend to all three tickers, which feeds `risk_posture` that the Macro Structural Architect agent reads directly. Worse than the original crash-to-`UNKNOWN` behavior it was replacing. v1.2 now raises per-ticker instead, so each ticker independently falls to `UNKNOWN` rather than sharing wrong data.
3. **`performance_auditor.py`'s claimed bug doesn't exist in this system.** The comment claimed "for SHORT sessions BD > BO" — false here: `bo_price`/`bd_price` are the session-level SSOT `breakout_trigger`/`breakdown_trigger` (`kabroda_mas_flow.py:1745-46`), always `bo >= bd` regardless of trade direction (CLAUDE.md's own inviolable definition — bo/bd are upper/lower session anchors, not direction-specific entries). The `abs()` is a harmless no-op; the comment now correctly documents why instead of asserting something false about the architecture.
4. **`bold-hubble/kqal/db_reader.py` imported `psycopg2`**, not installed and not in `requirements.txt` (this project uses `psycopg` v3 exclusively). Converted the module to `psycopg` v3 directly (`conn.autocommit`/`conn.read_only` properties instead of `set_session()`, `row_factory=psycopg.rows.dict_row` instead of `cursor_factory=psycopg2.extras.RealDictCursor`) rather than adding a second, redundant Postgres driver to `requirements.txt`. Verified both the psycopg conversion (imports and runs cleanly, fails gracefully with no `DATABASE_URL` set locally, matching this project's own standing local-dev limitation) and the yfinance fix (ran live against real market data — all three tickers resolved correctly and independently).

**Scope discipline:** `bold-hubble/kqal/` contains a much larger system (`alignment_engine.py`, `dashboard.py`, `system_auditor.py`, `trade_validator.py`, `krown_signals.py`, `improvement_queue.py`, templates/, static/) that `__init__.py` itself pulls in — none of that was part of this handoff or reviewed here, and none of it was touched or committed.

**Discovered while staging: `bold-hubble/` is its own separate git repository, nested inside this one** (`bold-hubble/.git`, one trivial "Initial commit," no remote configured at all). `git add` on files inside a nested repo from the outer repo silently does nothing — no error, no staging — which is exactly what happened when staging `bold-hubble/kqal/timeframe_analyzer.py` and `db_reader.py`. **Only `market_context_oracle.py` and `performance_auditor.py` (both at the outer repo's root) were actually committed and pushed to `kabroda.com`'s real GitHub remote.** The two fixed `kqal/` files exist correctly on disk with all the fixes applied and verified (import-tested, run-tested), but are not committed anywhere yet — `bold-hubble/`'s nested repo has no remote to push to, and folding it into the outer repo properly (removing the nested `.git`, or setting it up as a real submodule) is a decision for the owner, not something to do unilaterally. Flagged back to the owner rather than guessed at.

**TradingView "Mission Plotter" HUD showing DATA MISSING for 4H/1H candidates — real root cause found and fixed, plus a second, older bug it uncovered (2026-07-14).** Owner reported the Pine Script indicator (`Kabroda Mission Plotter v1.2`, lives in their TradingView account, not this repo) wasn't drawing entry/stop/target lines or the MACRO/MICRO HUD when copying a 4H/1H candidate. Owner pasted the actual Pine source this time, which made the diagnosis exact rather than guesswork: the indicator requires **exactly 9 pipe-delimited fields** (`bias|status|entry|stop|tp1|tp2|tp3|macro|micro`), gated on `array.size(parts) >= 9` — anything short of 9 and the whole HUD/line-draw block is skipped silently.

Traced both site-side copy paths against that contract:
- **4H/1H "COPY" button** (`copyTfLevel()`): produced `LONG|1H|Entry:62671.7|Stop:61813.47|Target:63529.93` — 5 fields, with text labels baked into the numbers (`"Entry:62671.7"` fails Pine's `str.tonumber()`), no T2/T3, wrong field meanings. Never actually built to feed this indicator.
- **4H/1H "COCKPIT →" button** (`openTfCockpit()`): produced a 7-field key — 2 short of 9, so `>= 9` fails outright. This was the owner's exact screenshot.
- **15M's own dedicated HUD key** (`renderSnapshotGrid()`, the one actually named "COPY TO HUD" in the UI): also only 7 fields (`bias|SA_APPROVED|entry|stop|t1|t2|t3` — no macro/micro at all). This is the bug a 2026-06-19 WORK_LOG note flagged as a risk ("HUD key is 7 fields... unverified against TradingView Pine parser") and never actually confirmed — now confirmed broken, and it's been silently producing a key the indicator has never once accepted.
- One more gap at the source: `market_radar.py`'s `_tf_candidate_verdict()` only ever returned `t1` — T2/T3 were never surfaced from `CampaignLog` into the radar payload for 4H/1H, even though v4 populates them on every row.

**Fix:** `_tf_candidate_verdict()` now also returns `t2`, `t3`, `macro_bias`, `dominant_direction` (all already stored on the row — `macro_bias` from `_calculate_weekly_force()`, `dominant_direction` from this week's confluence-capture work — no new computation needed). New shared JS helper `buildMissionKey9()` in `market_radar.html` builds the correct 9-field key from real data with safe `NEUTRAL`/`WAITING`/`0.00` fallbacks; `openTfCockpit()`, `copyTfLevel()`, and the 15M `renderSnapshotGrid()` key all now call it instead of three different, each-broken-in-its-own-way string builders. For 15M (poll-driven, must stay DB-only per its own Phase 1 design), MACRO/MICRO both reuse the already-cached `mtf_cached.confluence_direction` rather than adding a live fetch to every render tick; for 4H/1H (click-driven, no polling concern), the real per-candidate `macro_bias`/`dominant_direction` are used directly, no extra API call needed either way.

**Not verified live yet** — this needs the owner to actually paste a fresh 9-field key into the real TradingView indicator to confirm the lines/HUD draw correctly; I can verify the string shape and backend correctness but not the actual chart render.

**v4 stop/target asymmetry — Phase A backtest complete, real data contradicts the proposed fix, Phase B (production change) does NOT ship (2026-07-12).** Owner flagged a real, recurring pattern across several 4H/1H candidates (worst examples: 1H #143, stop $2,473 away vs. T1 only $1,184 away; 4H #136, stop $5,858 away vs. T1 only $1,471 away) and asked explicitly for a long-term architectural fix, not a parameter patch — see the full APPENDIX (2026-07-12) in the plan file for the complete diagnosis. Root cause confirmed exactly from the code: the stop is anchored to the nearest recency-selected pivot with no distance cap, while the target leg gets capped to `3×ATR14` whenever that pivot's distance exceeds `5×ATR14` — the stop is never revisited when the cap fires, breaking CLAUDE.md's Measured Move "distance is one number, used symmetrically" premise. The architecturally principled fix: when the cap would fire, treat the pivot as unusable for the stop too, and construct **both** stop and target from the same `ATR_FALLBACK` formula already used for the "no pivot found" case — restoring true 1:1 symmetry instead of an arbitrary cap tweak.

**Per the plan's own gate, this was NOT shipped to production before being backtested.** Extended `mtf_backtest_lab.py` (`build_trade_plan_windowed()` gained a `symmetric_fallback` param + ATR_FALLBACK parity fix for the no-pivot case; new `compare_symmetry()`/`print_symmetry_comparison()` + `--compare-symmetry` CLI mode) and ran it against real MEXC history at the exact production window sizes (48 bars/1H, 30 bars/4H — the already-chosen item #1 values, not re-tested). Real results, both timeframes:

- **1H** (90 days, N=162 total signals, cap fired on 22 = 13.6%): CAP-AFFECTED subset — v4 (today, asymmetric): 54.5% win, **+0.277R avg**. v5 (symmetric candidate): 36.4% win, **-0.069R avg**. v5 is worse.
- **4H** (300 days, N=126 total signals, cap fired on 22 = 17.5%): CAP-AFFECTED subset — v4 (today, asymmetric): 54.5% win, **+0.301R avg**. v5 (symmetric candidate): 45.5% win, **+0.021R avg**. v5 is worse again, same direction.

Both samples sit at N=22, below this project's own N≥30 bar individually — but the same direction repeating independently across two different timeframes and two different historical windows is a real, non-trivial cross-check, even short of statistical proof. **Decision: do not implement the Phase B production change (`gravity_engine.py` v5 cutover).** The architectural reasoning was sound on paper, but real backtested behavior says a wide, pivot-anchored stop gives these trades genuine room to reach even a capped target more often than a tight, ATR-matched stop does — forcibly symmetrizing hurts realized R in both tested samples. This is the backtest-before-ship gate working exactly as designed: catching a well-reasoned but wrong hypothesis before it touched live trade construction.

**The real, deeper gap this surfaced: this system has no position-sizing mechanism at all.** `CampaignLog.total_contracts` is hardcoded `0.0` at every creation site, confirmed via repo-wide grep to be never read anywhere. The R-multiple backtest above is a fair comparison *only* if position size is scaled inversely to stop distance (smaller size on a wider stop, so real dollar risk stays constant) — that's the standard, correct way an R-multiple system is supposed to be traded. Without that, a wide-stop trade that does eventually lose costs disproportionately more real dollars than the "-1R" bookkeeping implies at any fixed position size. This is a separate, real, currently-unaddressed initiative — not something to fold into this fix, and not scoped or built today. Worth its own future appendix once the owner wants to take it on.

`mtf_backtest_lab.py`'s extensions are left in the repo (harmless, standalone, not wired into production) for anyone re-running this comparison later as more real 4H/1H history accumulates in `gravity_memory` (a wider recency window of confirmed pivots over time could shift this result — worth re-checking periodically, not assumed permanent).

**Executive Dashboard + Confluence page audit, four fixes shipped (2026-07-09).** Owner asked directly whether the Executive Dashboard (`/suite/dashboard`) had ever been audited — is everything on it correct and does it need to be there — plus whether the new Confluence page could use a plain-English "what does this mean" section. Read the actual route code rather than eyeballing screenshots; found two genuine, pre-existing bugs unrelated to anything shipped this week, one already-correct-but-confusing metric, and one real UX gap on the page built earlier today.

1. **"Net R Lifetime" KPI and the Cumulative Performance chart were silently wrong** — both computed R as `wins - losses` (a win/loss COUNT), i.e. hardcoded +1.0/-1.0 per trade, completely ignoring the real `realized_pnl` field. This is exactly the assumption CLAUDE.md rule 5 and the 2026-07-04/05 `_frac_r()` fix both explicitly reject — the ledger has been storing correct fractional R (e.g. the -0.0858R and -0.0467R 4H expiries from this week) for days, but the dashboard never got updated to read it. Fixed in `main.py`: `/api/dashboard/overview`'s `net_r` now sums real `realized_pnl` (`CLOSED_WIN`/`CLOSED_LOSS`/`CLOSED_AT_EXPIRY`, `realized_pnl IS NOT NULL`); `/api/dashboard/mas-history`'s `pnl_series` and `trades_data.realized_pnl` do the same instead of the old ternary. `CLOSED_AT_EXPIRY` is now included in both (a real filled outcome with real fractional R) where it was silently excluded before; `EXPIRED` (never filled, "no trade") correctly stays excluded. `templates/suite_dashboard.html`'s trade-table row coloring updated from exact-string-match (`'+1.0R'`) to a sign-prefix check, since the R value is now real precision, not a fixed string.
2. **"4H Outcome vs. Session Bias" was mislabeled — it was never showing 4H data.** It read `DecisionJournal.kinematic_grade`, the old 15M radar-scan-level field (Crown Surgery Cut 2), entirely unrelated to the actual 4H/1H candidate system's own `kinematic_grade` (the field behind `audit_ai.py`'s H7, on `CampaignLog`). Rewired `/api/dashboard/accuracy`'s `grade_rows` query to pull real `CampaignLog` rows scoped to `session_timeframe IN (4H, 1H)`, using the same "correct = closed net-positive R" definition H7 already uses. Chart subtitle corrected to say what it actually shows; N is now shown directly in each bar's axis label (`PRIMED (N=3)` etc.) rather than hidden, since real N here is still thin — matches this project's own "no numbers without N" discipline. The Score 0-5 confluence-accuracy chart was left untouched — it reads a different, already-populated `DecisionJournal.confluence_score` field, not diagnosed as broken.
3. **"0% Cache Hit Rate" investigated — code is correct, not a bug.** Traced `agent_core.py`: `cache_control: {"type": "ephemeral"}` is genuinely set on every system prompt, and `cache_read_input_tokens`/`cache_creation_input_tokens` are genuinely captured from the API response into `AgentRunLog`. The near-zero reading is a structural consequence, not broken code: Anthropic's ephemeral cache has a 5-minute TTL, and almost every Kabroda agent (Senior Analyst, gravity/MTF interpreters, Elliott Wave, JEWEL specialist, performance auditor, publisher) fires at most once per session, hours apart — there's essentially never a second call to the identical system prompt within 5 minutes for a cache read to land. The two scenarios that legitimately could produce a read — the Senior Analyst's same-run JSON-parse retry, and the live SA commlink chat (`agent_name="senior_analyst_commlink"`, same system prompt reused across a live conversation) — are real but occasional/usage-dependent, and get diluted to invisible in an aggregate-across-all-agents number. Did not touch the caching mechanism itself (removing it would regress the one feature — commlink chat — that can genuinely benefit). Added a hover tooltip on the KPI card explaining why near-zero is expected, so it reads as "structurally normal" rather than "something's broken."
4. **Confluence page: added a plain-English "What This Means" card + fixed a real labeling bug found while auditing my own work.** New deterministic (no LLM, no cost — same Bucket-A pattern used everywhere else in this codebase) `buildPlainRead()` in `templates/confluence.html` synthesizes 2-4 sentences from fields already on the page: overall lean + which timeframes agree, an explicit callout when Daily/Weekly (the two largest timeframes) oppose the shorter-term lean, and a plain-English overextension/divergence warning. Footer line makes clear it's context only, not a trade signal — the Senior Analyst remains the only source of an actual verdict. Separately, screenshotting my own page surfaced a real bug: the RSI-divergence flag chip renders bare (`BULLISH`, `HIDDEN BEARISH`) right next to the bias chip, which can read as the page contradicting itself (e.g. bias chip says BEARISH, divergence flag says bare BULLISH) when it's actually a distinct signal (an early reversal warning, not a restatement of trend). Fixed by appending "DIVERGENCE" to the flag label.

Verified via the real entry point (`python -c "import main"`, clean) before pushing, not just `py_compile` — standing discipline since the circular-import incident earlier today.

**Deploy break + fix, same push (2026-07-09): circular import from the new `mtf_confluence_scanner` module-level import in `gravity_engine.py`.** The confluence-capture commit above added `import mtf_confluence_scanner` at the top of `gravity_engine.py`. Missed at review time: `battlebox_pipeline.py` already imports `gravity_engine` at module level, and `mtf_confluence_scanner.py` imports from `battlebox_pipeline` at module level — a three-module cycle. Render's boot log showed the real error immediately: `ImportError: cannot import name 'fetch_live_15m' from partially initialized module 'battlebox_pipeline'`. `python -m py_compile` (run before the original push) cannot catch this class of bug — syntax is valid, only the actual import chain at runtime breaks. **Fix, not a workaround:** moved the `mtf_confluence_scanner` import from module level to a lazy import inside `run_gravity_ingestion_loop()` itself — by the time the background loop actually runs, all three modules have finished initializing, so the cycle never triggers. No `try/except` swallow, no functionality removed. Verified properly this time: `python -c "import main"` (the actual boot entry point, not just the one broken module) run locally before pushing — clean, zero output. **Standing lesson:** for any new top-level import added to a module that's part of an existing import chain, verify with the real entry point (`import main`), not just `py_compile` on the changed file — compile-checking only catches syntax errors, not circular-import breaks that only surface when the actual module graph is walked.

**Multi-timeframe confluence exposed as its own view + fed into the daily audit as H10 (2026-07-09).** Owner's repeated "track everything, multi-timeframe" brainstorm (weekly/daily key levels, cross-timeframe conflict, poker-player-watching-the-whole-table framing) turned out to be mostly already built and just never surfaced: `mtf_confluence_scanner.run_mtf_confluence_scan()` already runs a live, session-lock-independent 5-TF (15M/1H/4H/1D/1W) direction vote with its own "aligned vs. opposing" summary language — confirmed via research before scoping any new work, the same discipline that caught the Component 6/`harness/` near-duplication the day before. It was only ever reachable bundled inside Market Radar's scan response. **Part A (exposure, not computation):** new `GET /api/confluence` route (`main.py`) calls the scanner directly and additionally surfaces the macro engine's independent weekly-200-SMA trend read (`battlebox_pipeline._fetch_weekly_200sma()`, 24h cadence) alongside the scanner's own EMA21/55-vote-based weekly/daily read — two different, previously un-reconciled trend definitions that already coexisted in this codebase, now shown side by side and clearly labeled instead of silently picking one. New standalone page `templates/confluence.html` (built against the real `site.css` token system — `var(--bg-card)`, `var(--accent)`, etc. — not invented colors) + `GET /suite/confluence` route + nav link. **Part B (extends Component 6, no new statistical infra):** `gravity_engine.py`'s two detectors now call the confluence scanner once per loop tick (shared between both, one call not two) and stash `dominant_direction`/`confluence_score` onto the `CampaignLog` row at candidate-creation time — two new nullable columns, same migration pattern as `macro_bias`/`kinematic_grade`. New `H10_TF_AGREEMENT` in `audit_ai.py`: does the live confluence read agreeing vs. opposing the candidate's own bias at fire time correlate with outcome — reuses `harness.tier_labels`/`harness.binomial_checkpoint` exactly like H7-H9, no new statistics code. Record-only, does not gate candidate creation. **Part C (economic calendar/FOMC tracking) deliberately not built this pass** — genuinely new territory (zero existing capability, confirmed via repo-wide grep), needs its own future research pass on which calendar data source to use before scoping.

**First real bug caught by the new date-range export/tracking tool, same day it shipped (2026-07-09).** Owner pulled `/admin/export-audit-ledger` with a date range the morning after Component 6's first live firing and noticed `daily_digests`/`audit_suggestions` showed suspiciously empty 4H/1H coverage despite real candidates existing that day. Root cause: `audit_ai.py`'s four queries all filtered `is_canonical == True`, copied from the 15M convention without checking it applies to 4H/1H rows -- it doesn't. `gravity_engine.py`'s `_detect_4h_bos`/`_detect_1h_bos` explicitly set `is_canonical=False` on every candidate row by design (keeps candidates out of the 15M track record's KPIs, not a legacy-data flag). This silently zeroed out 100% of the 4H/1H population the daily digest and H7/H8/H9 exist to examine -- would have stayed at N=0 forever with no error, if not caught. Fixed with a new `_real_btc_row()` filter (`symbol=="BTC/USDT" AND (is_canonical==True OR session_timeframe in 4H/1H)`) applied consistently across all four sites, plus a `target_logic_version=="v4"` filter added to H7 specifically (kinematic_grade/energy_grade only meaningful post-fix). Re-verified against synthetic data: the 4H row that was silently excluded before now correctly appears in both the digest and H8's count. This is exactly the loop the owner asked for working as intended -- track everything, make it reviewable in one click, and real problems get caught within a day instead of silently producing "no data" forever.

**KULTI LTI pulled from the live site, pending a from-scratch rebuild (2026-07-08, same day it shipped).** Owner's read, after further discussion: the design mixed the active-trading system's paradigms into what should have been a from-first-principles long-term investing system — confluence-count tiers shaped like the trading system's own gating logic, the "Krown Cross" component reusing `_build_jewel_reading()` (built and vocabulary-tuned for 15M intraday entry signals, not a monthly macro read), and — the sharpest catch — applying N≥30 statistical validation thinking (the exact discipline that correctly governs the 15M/4H/1H trading systems) to a strategy that can never accumulate 30 independent trials in a human lifetime. Trading and investing are different disciplines for a reason; this was built using the wrong one's toolkit. Nav link removed, scheduler unregistered, `GET /suite/lti` redirects to the dashboard, `POST /api/lti/protocol` returns 410 — same closure shape already used for `/register`. No data lost (both tables were empty; the first scheduled audit never fired). Files/tables left in place, not deleted — the real indicator math and the honestly-sourced Hash Ribbons work may still be worth reusing once the actual investing philosophy (Crown's trim-at-extremes vs. Mark Moss's never-sell-borrow-against-it vs. some deliberate blend) is settled, which is a strategy conversation, not a coding one. **Do not rebuild until that's resolved with the owner.**

**CORRECTION to the 2026-07-07 KULTI entry below:** it claimed the wave-label lookup was "confirmed working end to end" against real production data. That claim is now uncertain — discovered 2026-07-08 that the local dev shell used for standalone testing this whole session has **no `DATABASE_URL` set** (confirmed via direct `os.environ` check), meaning any local test reading the database falls back to an empty local sqlite file, not the real production Postgres. External-API-based checks (Kraken OHLCV, blockchain.com hash rate, Fear & Greed) were genuinely real regardless, since those don't depend on `DATABASE_URL` — but the wave-label DB read specifically cannot be re-confirmed as having hit production from that test. Not a functional bug (the query code itself is correct, confirmed again today via synthetic local data), just a correction to an overstated verification claim. Going forward: DB-dependent "verified against real data" claims from a local shell need this caveat unless `DATABASE_URL` is explicitly confirmed set first.

**Component 6 (Daily System Audit) built — but only after nearly duplicating a whole existing subsystem (2026-07-08).** Owner asked for the Audit-AI system (already spec'd in this master plan's Component 6, 2026-06-29, never built) to actually run, daily, covering all three timeframes with per-trade "why," pullable as a date-range JSON export. Mid-implementation, a duplicate-table crash (`Base.metadata.create_all()` refusing a second `AuditSuggestionLog` class) caught a real mistake: a full `harness/` audit ecosystem already existed — `query_layer.py`, `join_logic.py`, `tier_labels.py` (real four-tier evidence system), `binomial_checkpoint.py` (real one-tailed significance testing, writes to `TrialsLog`), `baseline.py`, `audit_runner.py` (H1-H6 hypotheses against `SessionAuditLog`, writes to the *real* `AuditSuggestionLog`) — more rigorous than what was being built from scratch. Stopped, researched it properly (confirmed via repo-wide grep: zero `harness/` code touches `session_timeframe`/4H/1H — that territory was genuinely untouched), and rebuilt the plan to integrate rather than duplicate. Full detail in the "COMPONENT 6" entry below.

**KULTI Long-Term Investing module shipped (2026-07-07) — brand-new, separate, advisory-only module.** Eric Crown's "KULTI" course (confluence-count BTC framework) implemented end-to-end: `hash_ribbons.py` (new, real Capriole Hash Ribbons via blockchain.com's free API), `lti_engine.py` (9 components computed on native Kraken weekly candles + Crown's confluence engine + a Kabroda-native gravity-wall cross-confirmation), `lti_interpreter.py` + `agents/lti_interpreter.md` (new Bucket B AI judgment layer synthesizing the confluence count with Kabroda's own Elliott Wave macro position), two new DB tables (`LtiCheckpoint` append-only, `LtiProtocol` mutable settings row), a new monthly scheduler in `main.py`, `GET /suite/lti` + `POST /api/lti/protocol`, and `templates/lti.html`. Every component verified against real live data before wiring together (hash rate API, Kraken weekly OHLCV genuinely live; wave-label DB read's production status uncertain, see correction above). Full detail in the "KULTI LONG-TERM INVESTING MODULE" entry below.

**Full backend + frontend alignment audit (Artifact) → three real fixes shipped same session.** Requested audit surfaced: (1) `GET /register` rendered a missing `register.html`, linked from nav/login/base — any click crashed to a raw traceback; traced the handler and found it was leftover membership-commerce machinery (auto-login + hardcoded Whop checkout redirect) from a past era. (2) Macro War Room's asset selector offered ETH/USDT and SOL/USDT, but the CCO brief/trade-plan panel can never populate for them — confirmed via owner: ETH/SOL were part of the old setup, deliberately dropped when the team began building the AI system to focus entirely on BTC. (3) No admin "add user" capability existed — only delete/reset-password/toggle-role for existing accounts.

**Fixed, one session, per owner direction (site is invite-only, not offering memberships right now):** `auth.py`'s `/register` GET+POST now redirect to `/login` (route stays defined so no dangling crash for stale links, but fully inert — no template to maintain). Removed the "Start 7-Day Free Trial" (`base.html`), "JOIN NOW" (`nav.html`), and "Initialize Account" (`login.html`) links entirely. Added `POST /admin/create-user` (`main.py`, same pattern as the existing `delete-user`/`toggle-role`/`reset-password-manual` routes — `get_db` + admin check + direct `UserModel` creation via `auth.hash_password()`, `subscription_status="active"`, no Whop step) plus a "+ ADD USER" button/modal in `admin.html` reusing the exact existing `.modal-overlay`/`.inp-pass`/`.btn-save` CSS and JS pattern already used for "Reset Key." Trimmed Macro War Room's selector to a single BTC/USDT option, matching Gravity Map's already-correct sibling pattern.

**Also confirmed via direct grep (not assumed):** `mtf_backtest_lab.py`/`verify_r_accounting.py`/`verify_close_vs_wick.py` have zero imports and zero routes anywhere — fully isolated from the live app, exactly as intended.

**Not touched, flagged for owner:** `mission_brief.html` (admin-only static reference) still describes ETH/SOL as actively-governed tradeable assets with specific rules — content/copy decision, not code, left for the owner to decide (keep as history, add a "paused" caveat, or remove).

**KULTI, elaborated further (still parked, not built):** owner now frames this as a future membership tier — Crown's own philosophy on when to trim profits vs. add back in over a long horizon, bundling in the daily publication, plus an interactive version of Crown's own checklist to help someone set up and learn. Richer shape than the original course-port idea; still explicitly "later," not now.

**Item #3 (Senior Analyst never receives explicit "current price") shipped — the last original punch-list item.** Corrected the trace from the original 2026-07-03 diagnosis: the actual gap was NOT in `get_live_battlebox()`'s outer wrapper (that's a separate consumer) but in `_compute_sse_packet()` (`battlebox_pipeline.py:607-666`) — it already computes `last_price` locally (line 620, the most recent 5m close within the 24h pre-lock window) but never threaded it into `computed["context"]` before returning the packet `run_mas_analysis()` actually consumes. Two-line fix: `computed["context"]["current_price"] = last_price` in `_compute_sse_packet()`, plus one explicit `f"Current Price (at lock): ${current_price:,.2f}"` line added to `_build_senior_analyst_context()`'s output text (`kabroda_mas_flow.py`), right under the Symbol/Date/Session header. Pure additive text/context change — no trade-construction logic touched, no schema change, no new column. Both files verified via `ast.parse` (note: use `encoding='utf-8'` explicitly on Windows — the default `open()` encoding chokes on this repo's em-dashes and throws a misleading `UnicodeDecodeError` that looks like a syntax problem but isn't).
...
**DEPLOY + FIX VERIFIED LIVE (2026-07-06).** Render boot clean, no errors, service live. Pre-fix baseline confirmed via direct query: `168H_MICRO_ANCHOR` had **1,685 permanently-active rows per symbol** (BTC/ETH/SOL) — larger than the initial small sample suggested, confirming the bug's real scale. Re-queried after the loop's next natural hourly write: **dropped to exactly 1 active row per symbol**, precisely as predicted — direct, clean confirmation the deactivate-before-insert fix works correctly in production. `7_DAY_KABRODA` (480/252/252) and `1W_MACRO_ANCHOR` (12/12/12 each) unchanged, as expected — daily and weekly cadence respectively, haven't hit their next write cycle yet. **Carry-forward, no action needed, just check later:** `7_DAY_KABRODA` should clean up at the next session lock (~9 AM ET); `1W_MACRO_ANCHOR` next Sunday.

**Owner directive (2026-07-06): pause new work, let today's fixes accumulate real trade data before continuing.** Explicit preference: wants to see solid, clean, actionable trades showing up in the radar and build real confidence the system finds high-probability setups, rather than continuing to stack new fixes. Item #3 (Senior Analyst never receives explicit "current price" — the last untouched original punch-list item, small/contained) stays queued, not started. Next real work session should pick up there, or continue watching how today's shipped fixes (v4 stop/target, R-accounting, stop-hunt buffer, macro-bias gate, kinematic_grade, gravity-wall cleanup) perform on genuinely new live data first.

**Runner mechanic — shadow-mode partial-profit trailing shipped, 15M only, record-only (2026-07-06, same day).** Owner picked this up next (site-audit/registration cleanup landed first, see that entry above). Full detail in "RUNNER MECHANIC" entry below.

**Runner mechanic, part 2 — extended to 4H/1H via structural-zone trailing (2026-07-07).** Owner watched a real 4H candidate fire, hand-verified its Fibonacci targets, and correctly pushed back on scoping the runner to 15M-only. Full detail in "RUNNER MECHANIC, PART 2" entry below.

### RUNNER MECHANIC — SHADOW-MODE PARTIAL-PROFIT TRAILING, 15M ONLY, RECORD-ONLY (2026-07-06)

**What it is:** the 15M system currently exits the whole trade at T1 (the 1× measured-move). Three independently-sourced descriptions (external bold-hubble research, Mafioso's Meta Signals alert examples, this project's own original master plan Component 4) converge on the same shape: bank ~50% at T1, move the remainder's stop to breakeven, trail it toward T2/T3 via a 21 EMA instead of taking the full exit. Owner picked this up as "item 1" from the post-audit "what's next" list.

**Two scope decisions, both owner-confirmed before any code was written:** (1) **15M only** — no EMA21 infrastructure exists for the 4H/1H candidate system, and 4H/1H is still record-only/unvalidated, not a live signal; porting there is explicitly out of scope for this pass. (2) **Shadow mode, not live** — real 15M trades keep closing and recording exactly as they do today at T1 (`status`/`realized_pnl`/`closed_at` completely untouched). In parallel, the engine tracks a shadow runner leg on new, separate nullable columns, modeling what the runner *would have* produced, for the owner to review before ever flipping it live.

**A real design bug was caught and designed around before writing code, not after:** naively recomputing "the current EMA21" once per 60-second tick and applying it as the stop across the whole re-scanned candle window is look-ahead bias — a candle from three hours ago would get tested against a stop level the EMA hadn't reached yet at that historical moment. Fixed by walking candles strictly chronologically and only ratcheting the stop using 15-minute bars that have already closed as of the candle being tested — the same principle Phase 2 already uses when scanning candles in order.

**Implementation (`ledger_closing_engine.py`, `database.py`):** ten new nullable `CampaignLog` columns (`shadow_runner_active`, `shadow_runner_stop`, `shadow_runner_ema21`, `shadow_runner_bucket_ts`, `shadow_runner_bucket_close`, `shadow_runner_last_scan_ts`, `shadow_runner_closed_at`, `shadow_runner_exit_reason`, `shadow_runner_leg2_r`, `shadow_runner_blended_r`). Phase 2's existing T1-hit branch gains three new lines (seeds `shadow_runner_active=True`, `shadow_runner_stop=entry_price`, `shadow_runner_last_scan_ts=candle_ts`) alongside the unchanged real-close fields — Phase 4's near-identical T1 block is untouched, confirmed by anchoring the edit to Phase 2's own "P2" log tag so it couldn't land in the wrong block. New "Phase 3B — Shadow Runner Tracking" section reuses the existing `_fetch_1m_since()` helper as-is (no second fetch call, no new exchange dependency) — resamples the returned 1-minute candles into 15-minute buckets on the fly, updates a running EMA21 via a tiny incremental formula (`new_ema = prev_ema + (2/22)×(close−prev_ema)`, seeded with the first bucket's close rather than a full 21-bar SMA warm-up — an acknowledged simplification for a shadow/trial feature), ratchets `shadow_runner_stop` only in the favorable direction (never below breakeven), and resolves the row on a stop touch, a T3 touch, or a 5-calendar-day time cap (matching the master plan's original Component 4 spec). `total_contracts` (confirmed hard-coded `0.0` everywhere, never read anywhere in the codebase) has no bearing on this — since `ledger_closing_engine.py` is a monitoring/audit engine, not a live order-execution engine, "close 50%" is modeled purely as `shadow_runner_blended_r = 0.5×realized_pnl + 0.5×shadow_runner_leg2_r`, not a real partial fill.

**Not touched:** Phase 4 (4H/1H candidates) — explicitly out of scope; Phase 3 (existing post-T1 T2/T3 observation, ticker-only, same-session-only) — left exactly as-is, it's a different mechanism serving a different purpose and isn't suitable to extend for a multi-day trail; the real Measured Move formula, the 15M acceptance gate, and the real stop-assignment logic — none of this touches trade construction, only what happens after a real T1 close.

**Verification plan (not yet run — feature just shipped):** immediate check on the next real T1 hit post-deploy (`shadow_runner_active=true`, `shadow_runner_stop=entry_price` populated); the real payoff check is `SELECT ... WHERE shadow_runner_closed_at IS NOT NULL` once shadow rows start resolving over the following days/weeks (bounded by the 5-day time cap) — comparing real `realized_pnl` against `shadow_runner_blended_r` is the evidence base for any future, separate decision about going live. This is inherently a multi-day-to-multi-week wait, not a same-session check.

### RUNNER MECHANIC, PART 2 — 4H/1H STRUCTURAL-ZONE TRAILING, SHADOW MODE (2026-07-07)

**Why this got built the same week:** owner watched a real 4H LONG candidate fire (entry $64,027.60 / stop $62,256.38 / T1 $65,798.82 / T2 $66,893.43 / T3 $68,664.65 — hand-verified against the v4 Fibonacci formula, all three match exactly) and asked why the runner mechanic wasn't tracking 4H/1H too, since "we would want to track everything." Fair pushback — the "not real money yet" reasoning given for scoping to 15M-only didn't actually hold, since shadow tracking is observational either way on both systems. The real reason was more concrete: no EMA21 exists for 4H/1H, and — more importantly — the original master plan's own Component 4 spec calls for 4H/1H to trail using `gravity_memory` structural zones (supply/demand pivots), not an EMA, since a 4H/1H swing is slow enough that a moving-average trail may be the wrong tool for that timescale. Owner chose to design that properly rather than just port the EMA mechanism or leave the gap open.

**Real, useful discovery: zero new database columns needed.** The ten `shadow_runner_*` columns added for the 15M version are generic — Phase 3B (15M) filters on `session_timeframe=="15M"`, this new Phase 4B filters on `mas_approval_status IN ("4H_CANDIDATE","1H_CANDIDATE")` — mutually exclusive populations by construction, so the same columns serve both mechanisms safely. The EMA-specific columns (`shadow_runner_ema21`/`_bucket_ts`/`_bucket_close`) simply stay NULL on 4H/1H rows, same established pattern as `kinematic_grade`/`macro_bias` staying NULL on 15M rows today.

**Implementation (`ledger_closing_engine.py` only — no `database.py` changes this time):** Phase 4's shared CANDIDATE T1-hit branch (handles both 4H and 1H rows, distinguished by `session_timeframe`) gains the same three-line seed as Phase 2's did (`shadow_runner_active=True`, `shadow_runner_stop=entry_price`, `shadow_runner_last_scan_ts=candle_ts`). New "Phase 4B" section walks 1m candles chronologically (reusing `_fetch_1m_since()` as-is) and, per candle, queries a new `_nearest_zone_by_price()` helper against `gravity_memory` for the nearest qualifying structural zone in the trail direction (DEMAND below price for LONG, SUPPLY above price for SHORT) — deliberately **not** reusing `_nearest_pivot_in_window()` (item #1's helper), since that function's recency-first ordering was specifically chosen for stop *selection* at construction time and would be the wrong ordering here; a genuine price-proximity query was needed, patterned after `trade_structure_analyst.py`'s `_find_wall_long()`/`_find_wall_short()` shape (that code scans an in-memory KDE-peak list — this is the same idea as a live SQL query, since 4H/1H has no KDE peaks list). The zone query's price bound only admits zones strictly better than the current stop (enforcing "ratchet only favorably" directly in SQL) and filters `timestamp <= candle_ts` — the same no-look-ahead discipline the 15M EMA trail required, applied here so a zone detected after the candle being tested can't count. T3 stays the fixed, already-computed v4 Fibonacci target (only the stop trails, matching the 15M version's design). Time cap: 5 days (4H) / 2 days (1H) from the real T1 close — reusing the same per-timeframe durations already established for `STOP_WINDOW_4H`/`STOP_WINDOW_1H` rather than inventing new numbers.

**Not touched:** Phase 4's real `status`/`realized_pnl`/`closed_at`/`session_expires_at` handling; T1/T2/T3 construction (v4 Fibonacci staging, unchanged); Phase 3B / the 15M EMA mechanism (separate code path, untouched).

**Verification plan (not yet run):** same pattern as the 15M version — confirm `shadow_runner_active`/`shadow_runner_stop` seed correctly on the next 4H/1H T1 hit post-deploy, then watch for the shadow leg to actually ratchet as real `gravity_memory` zones qualify, then compare `realized_pnl` vs. `shadow_runner_blended_r` once shadow legs resolve (bounded by the 5d/2d time cap) — same evidence-before-action discipline as everything else this session.

### KULTI LONG-TERM INVESTING MODULE (2026-07-07)

**What it is:** a brand-new, architecturally separate module — a monthly-cadence, human-decision BTC accumulation/distribution framework based on Eric Crown's "KULTI" course (`bold-hubble/krown_courses/kulti/`, confirmed via direct course research this session). Not part of the 15M/4H/1H active-trading pipeline, never auto-executes anything — Crown's own framework states its boundary explicitly: "the framework flags WHEN, you decide HOW MUCH." Owner-confirmed scope: BTC only (matches Kabroda's existing ETH/SOL scope-narrowing), and the full "improved" version built now rather than a stripped-down v1 — an AI interpreter layer, gravity-wall cross-confirmation, and a real Hash Ribbons component were all owner-requested from day one, not deferred.

**Crown's actual system, confirmed from the course text (not assumed):** it's a confluence-COUNT gate, not a weighted score — 1-2 signals firing = No Action, 3 = Watch, 4 = Execute, 5 = Very High, 6+ = Generational Setup. 11 named components, but only some have real documented thresholds: BBWP (bottom/top 10-20%) and PMARP (<5%/>95%, using a 200 SMA for "macro investing" mode per the course's own instruction) are exact course numbers; RSI and the "Krown Cross" (21/55 EMA) are qualitative only ("oversold and turning up," "supportive or reclaimed") with no numeric threshold given; Percent-Below-High, Low Month Days, and Moon Phases have **zero** documented formula anywhere in the course. The exit/re-entry "Rule Trinity" is real and quotable: never fully exit (5% cash floor minimum), trim distribution-side winners to a 15% residual (never zero), monthly audit + quarterly rebalance vs. SPX.

**Honesty discipline applied throughout:** where Crown gives a real number, that number is used exactly. Where Crown gives only qualitative language, a clearly-labeled applied convention is used instead (e.g. RSI≤30-and-turning-up for accumulation) and marked in code comments as NOT Crown's own number. Where Crown gives nothing at all (Percent-Below-High, Low Month Days, Moon Phases), the raw value is computed and displayed but **not** counted toward the confluence fire tally — informational only, never a fabricated rule. Hash Ribbons (component #7) has zero formula in Crown's course either, but unlike the three above, a real public formula exists (Capriole Investments' 30D/60D hash-rate SMA cross) — implemented for real via a new `hash_ribbons.py` module hitting blockchain.com's free Charts API, honestly labeled as sourced from Capriole's public methodology, not Crown's undocumented internals.

**Kabroda-native additions (owner-requested, not in Crown's course):** (1) an AI interpreter layer (`lti_interpreter.py` + `agents/lti_interpreter.md`, mirrors `gravity_interpreter.py`'s exact Bucket-B shape) that reads the confluence count alongside Kabroda's own current Elliott Wave position (queried from `MacroNarrativeLog`) and produces qualitative judgment — e.g. distinguishing a genuinely *stacked* signal read from a *split* one, something Crown's pure count-gate can't do. (2) A gravity-wall cross-confirmation: does the current price sit near a macro-tier (`permanence_class=0`) `gravity_memory` structural cluster — a bonus confluence signal layered on top of Crown's 11, additive not replacing.

**Implementation, verified against real live data at every stage before wiring together:**
- `database.py`: `LtiCheckpoint` (append-only, one frozen row per monthly audit, matches `MacroNarrativeLog`/`InterpreterLog`'s write-once convention) and `LtiProtocol` (mutable single row — the user's editable "One-Page Protocol" checklist — matches how every other user-editable setting in this codebase works, not a versioned log). Both auto-created via `Base.metadata.create_all()`, no ALTER TABLE needed.
- `hash_ribbons.py` (new): live-tested against the real blockchain.com API — returned a real `CAPITULATION` classification with real MA30/MA60 values on first run.
- `lti_engine.py` (new): fetches native weekly BTC candles via `ccxt.kraken` (not MEXC — Kraken's listed BTC since 2013, giving real 252-week percentile depth; confirmed via research that resampling Kabroda's existing shallow ~1500-day daily fetch would only yield ~215 weekly bars, too thin). Reuses `battlebox_pipeline._calc_bbwp`/`_calc_pmarp`/`_calc_rsi`/`_calc_ema_series`/`_build_jewel_reading` directly rather than reimplementing. Sync module by design (plain `ccxt`, not `ccxt.async_support`) since it's invoked via `asyncio.to_thread()`, matching `elliott_wave_specialist.run_elliott_wave_analysis()`'s exact calling convention. Live-tested end to end: real BTC weekly data → BBWP 30.95 (NEUTRAL), PMARP 2.1 (EXTREME_DEPRESSED — a real accumulation-favoring signal), Krown Cross BEARISH_EXPANDING, confluence engine correctly resolved to NO_ACTION (2 fires, split read: PMARP accumulation vs. trend-structure distribution) — hand-verified the arithmetic matches.
- `lti_interpreter.py` + `agents/lti_interpreter.md` (new): mirrors `gravity_interpreter.py` exactly — context builder + `agent_core._call_from_spec("lti_interpreter", ...)`, fail-open, logged to the existing `InterpreterLog` table (`interpreter_name="lti_interpreter"`, no new log table). System prompt carries the same "describe, don't decide" mandate as every other interpreter, plus Crown's own explicit "flags WHEN, you decide HOW MUCH" boundary. Local-tested: correctly fail-opened to `None` when no `ANTHROPIC_API_KEY` is present (expected locally — real credentials only live in Render), proving the plumbing works; actual prompt output only verifiable once live.
- `main.py`: `_seconds_until_month_start()` (calendar-month boundary, not a rolling 30-day delta, so month-length variation doesn't drift the cadence) + `run_monthly_lti_scheduler()`, mirroring `run_weekly_scheduler()`'s exact shape (dedup-by-`created_at`, `asyncio.to_thread()`, registered in `lifespan()`). New `GET /suite/lti` (renders latest checkpoint + interpreter narrative + protocol form + 12-month history) and `POST /api/lti/protocol` (split from the page route, matching `/account` + `/account/settings`'s existing pattern). Confirmed `import main` succeeds cleanly with the new module wired in.
- `templates/lti.html` (new) + `templates/nav.html` (new "LONG-TERM" link): built against the site's real "Espresso Executive Theme" design tokens (`site.css`'s `:root` variables — `--win`/`--loss`/`--warn`/`--neutral` for conviction-label color-coding, `--font-mono`/`--font-body`), not invented colors. Test-rendered via Jinja2 directly in both empty-state (zero checkpoints yet) and populated-state — both render cleanly.

**Not touched:** the 15M/4H/1H active-trading pipeline (`gravity_engine.py`, `sse_engine.py`, `structure_state_engine.py`) — this module shares no code path with trade construction, by design, per the original scoping memory. No new external dependency added to `requirements.txt` — `hash_ribbons.py` uses stdlib `urllib.request` (same zero-dependency pattern as `external_intel_reporter.py`), and the weekly Kraken fetch reuses the already-installed `ccxt` package's plain (non-async) API.

**Not yet verified (can't be until real time passes):** the first live monthly firing (next calendar-month boundary), whether the AI interpreter's real prompt output is actually good (only testable with live Render credentials), and — inherently — the multi-year backtest this whole framework is supposed to be evaluated against, which by definition can't be rushed.

### COMPONENT 6 — DAILY SYSTEM AUDIT, INTEGRATED WITH THE EXISTING `harness/` ECOSYSTEM (2026-07-08)

**Motivation:** owner watched a real 4H/1H losing stretch (candidates 143/141/140/138/137/136: 4 resolved, all `CLOSED_LOSS`, every row `kinematic_grade=TANGLED`, never above `MODERATE` energy) and asked for the system to log/track/audit itself continuously — "like a poker player," always watching, not a system that runs once and gets left alone. This is Component 6 from the master plan (top of this file, designed 2026-06-29), never built until now. Clarified scope during planning: daily (not weekly) cadence, and a per-trade "why did this fire" record across all three timeframes — not just aggregate win-rate statistics.

**The real mid-build correction:** the first implementation pass designed a brand-new `AuditSuggestionLog` table and hypothesis-scanner from scratch. It crashed on `Base.metadata.create_all()` — `sqlalchemy.exc.InvalidRequestError: Table 'audit_suggestion_log' is already defined`. That crash caught something real: a full `harness/` audit ecosystem already existed (from an earlier session not covered in this session's carried-forward context) — `harness/query_layer.py` (read-only `CampaignLog`/`DecisionJournal`/`JewelSnapshotLog` accessor), `harness/join_logic.py` (builds "approved"/"stand-down" event streams), `harness/tier_labels.py` (the real four-tier evidence system: `DIRECTIONAL_OBSERVATION` N<30, `PRELIMINARY_SIGNAL` N 30-49, `PROVISIONAL_FINDING` N 50-99, `VALIDATED_EDGE` N100+), `harness/binomial_checkpoint.py` (real one-tailed significance testing at N milestones, logs to `TrialsLog`), `harness/baseline.py` (categorical-breakdown digest generator — "the ONLY harness component that runs now," per its own header, though "runs" meant manually), `harness/audit_runner.py` (H1-H6 hypotheses against `SessionAuditLog`, writes to the real `AuditSuggestionLog`, wired to `POST /api/admin/run-audit`), `harness/audit_writer.py`, `harness/health_check.py`, `harness/test_audit_safety.py`. Confirmed this is more statistically rigorous (real binomial significance testing, not naive win-rate comparison) than what was being drafted.

**Stopped, researched properly, redesigned to integrate rather than duplicate.** Two facts changed the plan: (1) confirmed via repo-wide grep that **nothing in `harness/` touches `session_timeframe`, `4H_CANDIDATE`, or `1H_CANDIDATE`** — the entire ecosystem is built exclusively around the 15M pipeline, so a 4H/1H-focused build has zero duplication risk. (2) confirmed via full `lifespan()` review that **nothing in `harness/` runs automatically anywhere** — `audit_runner.py`/`baseline.py`/`health_check.py` are all manual-invocation-only (Render Shell scripts or the single admin API route). This directly answered the owner's "can the system do this on its own" question: today, no, not even the already-built H1-H6 engine. That was the single highest-leverage fix available.

**Shipped, three parts:**
1. **Automated the already-built 15M engine.** `harness.audit_runner.main()` folded into the existing `run_weekly_scheduler()` as a third dedup-checked task — **kept at weekly cadence, deliberately not daily**, because `audit_runner.py`'s `consecutive_runs_surfaced` escalation ("3 consecutive runs at PROVISIONAL_FINDING+ → owner review") was designed against a weekly cadence; running it daily would cheapen "3 in a row" from 3 weeks to 3 days. Dedup uses a content check (`SystemAuditLog.audit_md.contains("AUDIT-AI WEEKLY LEDGER")`), not just symbol+date_key, since the Performance Auditor writes to the same table with the same key shape.
2. **New `audit_ai.py`: three hypotheses in the genuinely-untouched 4H/1H space**, daily — kinematic_grade/energy_grade correlation (H7, directly motivated by the live losing stretch), 4H macro-bias alignment vs. the 2026-07-06 backtested inversion (H8), shadow-runner-vs-real comparison (H9, formalizing the runner mechanic's own planned review checkpoint). Each computes a live win-rate + `harness.tier_labels.tier_label()` every day (always-on reading) and additionally fires `harness.binomial_checkpoint.run_checkpoint()` for real significance testing at N milestones — writes to the same `TrialsLog` the 15M side already uses. Suggestions logged to the **existing** `AuditSuggestionLog` (imported, not redefined) under new ids `H7_KINEMATIC_4H1H`/`H8_MACROBIAS_4H`/`H9_RUNNER`, continuing H1-H6's numbering, reusing `audit_runner.py`'s own update-existing-OPEN-row-or-insert pattern for consistency.
3. **New `DailyAuditLog` table + `build_daily_digest()`: the per-trade "why," across all three timeframes** — the one piece with no existing precedent anywhere in `harness/` (`baseline.py` does categorical breakdowns, not per-trade records). For every `CampaignLog` row created or closed on a given day: 15M rows surface `mas_executive_brief` (the CRO's own reasoning, already exists) + `structure_reasoning`; 1H/4H rows surface `macro_bias`/`kinematic_grade`/`energy_grade`/`htf_anchor_type`/`htf_anchor_price`/`target_logic_version`. Nothing computed or invented — pure surfacing of already-populated fields.

New daily scheduler (`run_daily_4h1h_audit_scheduler()`, 23:45 UTC, reuses the existing generic `_seconds_until_utc()` helper) runs parts 2+3 together. `GET /admin/export-audit-ledger` extended with optional `start_date`/`end_date` — when present, response grows to include `daily_digests`/`audit_suggestions`/`trials` alongside the raw `ledger`, covering every audit data source in one pull. New card on `templates/admin.html` (today's digest summary, recent findings table, date-range picker + "COPY JSON" button matching `research_lab.html`'s exact `copyLog()` pattern).

**Verified:** `audit_ai.py`'s logic tested against synthetic `CampaignLog` rows inserted into the local (gitignored, throwaway) sqlite fallback — H7 correctly distinguished a TANGLED+WEAK loser from a PRIMED+STRONG winner, H8 correctly scoped to 4H-only, H9 correctly reported "accumulating" with zero shadow legs, tier labels correctly came from the real `harness.tier_labels` module. `main.py` confirmed importing cleanly end-to-end with the full new wiring. Real production verification (does it find real patterns in real accumulated data) can only happen post-deploy.

**Discovered while testing (logged honestly, see the correction at the top of this file's NEXT SESSION START marker):** the local dev shell has no `DATABASE_URL` set, so standalone tests this session that read the database were hitting an empty local sqlite fallback, not production — the synthetic-data test above was designed around that constraint once discovered.

**Committed in 4 stages (`bc0ca6d` backend, `25791a2` scheduler+export route, `7f45970` admin UI, `5179bac` docs) and pushed 2026-07-08.** **DEPLOY VERIFIED (2026-07-08 18:55 UTC):** Render build succeeded, no boot errors. Boot log confirmed both new schedulers registered with correct next-fire math: `[SCHEDULER] Daily 4H/1H audit scheduler starting... next run in 4.8h (23:45 UTC)` and `[SCHEDULER] Weekly: next run in 100.1h (Sunday 23:00 UTC)` (now includes the newly-folded-in `harness.audit_runner.main()` alongside Elliott Wave/Performance Auditor). Gravity engine, trade-lifecycle monitor, macro engine scan, outcome tracker, monitor heartbeat all unaffected and clean. **Export tool confirmed working against real data same day:** owner pulled `/admin/export-audit-ledger` with a date range and got back 29 real, correctly-filtered `CampaignLog` rows spanning 2026-06-24 to 2026-07-08 (including id 143, the live SHORT candidate from the radar screenshot earlier this session). `daily_digests`/`audit_suggestions`/`trials` correctly returned empty — expected, since neither the daily (23:45 UTC) nor weekly (Sunday 23:00 UTC) job had fired yet at pull time, not a bug. **Still outstanding, carry-forward:** confirm the first real daily digest + H7/H8/H9 findings appear after tonight's 23:45 UTC run, and the first-ever automatic H1-H6 run after this Sunday's 23:00 UTC weekly fire — both are genuinely new automatic firings with no prior precedent to compare against, worth a deliberate look once each has run at least once.

**Not touched:** `harness/baseline.py`, `harness/health_check.py`, `harness/deferred_tests.py`, `harness/test_audit_safety.py` — all remain manual-only, out of scope for this pass; `harness/query_layer.py`/`join_logic.py`/`tier_labels.py`/`binomial_checkpoint.py` used as-is (imported, not modified).
...
**KEY WATCH HYPOTHESIS (N=3 — not acted on, not close to meaningful):** does a confluence-opposed 4H/1H candidate underperform confluence-agreed ones once N builds? Update 2026-07-14: the 2-for-2-losses read is already gone — id=157 (1H LONG, scanner=BEARISH, opposed) **won T1**. Confluence-opposed record is now 1W/2L. For contrast, the confluence-*agreed* population is mixed too: id=151 (agreed BULLISH) lost, id=147 (agreed BULLISH) closed at a small positive expiry, id=155 (agreed BEARISH, 4H) won T1. Nowhere near enough data on either side to read anything into this — H10 tracks it automatically; this entry is the human-readable cross-reference, not a duplicate mechanism.

**SECOND WATCH HYPOTHESIS, new 2026-07-14 (N=2 — not acted on):** does the shadow runner mechanic (ride past T1 with a trailing stop) beat taking the full T1-only exit? Two data points now point opposite directions. id=146 (2026-07-10): real T1-only exit **+1.5514R** beat the shadow runner's blended **+1.5105R** — real exit won. id=157 (2026-07-14): shadow runner rode to T3 and blended to **+1.809R**, beating the real T1-only exit of **+1.0R** — shadow runner won this time. H9_RUNNER tracks this automatically; flagging both instances by name since N=2 is too thin to even guess a direction yet.
