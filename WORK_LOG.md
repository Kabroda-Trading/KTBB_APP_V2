...
**2026-06-15 — Two read-only investigations: architecture honest-picture + energy/level time-coherence gap**
...
- **ENERGY/LEVEL TIME-COHERENCE GAP found and logged** (docs-only) — read-only trace of `battlebox_pipeline.get_live_battlebox()` confirmed: the pipeline fires on first page-visit after lock time (no background auto-fire at 9:30 AM). Levels (bo, bd) are always correctly bounded to the 9:00–9:30 AM calibration window. BUT the energy reads (fuel_gauge, JEWEL, RSI, MACD, ADX, micro_state, kde_peaks, macro_environment) are sampled fresh at the moment of the first page-visit — potentially 30+ minutes after lock. If first visit is 10:00 AM, agents analyze 9:30 AM level geometry against 10:00 AM momentum with no warning flag. Same integrity-risk family as the phantom-loss bug. Four candidate fixes logged in Suggestion Box; highest-value likely = auto-fire at lock time via scheduler (connects to W-12; one change, two wins — coherent data + brief waiting on arrival).
...
7. **[DATA-INTEGRITY — SCOPE NEXT]** ENERGY/LEVEL TIME-COHERENCE GAP (Suggestion Box 2026-06-15): decide fix approach and priority. Leading option: auto-fire pipeline at lock time via background task (W-12 scheduler hook already exists). Do NOT fix blind — scope the approach first, then build.
