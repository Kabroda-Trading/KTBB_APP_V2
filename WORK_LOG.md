...
**Item #5 (macro/weekly bias check) shipped, record-only per-timeframe as backtested.** Full detail in the "PUNCH-LIST ITEM #5" entry below. Item #6 (KDE-wall check) was reframed after owner pushback: before porting the 15M's HEAVY/MAXIMUM wall-snap mechanism to 4H/1H, investigate whether that mechanism actually works well on the 15M system it's already live on — owner's own observation is that walls often get "taken out with strong energy in the chart," suggesting the intensity classification or snap behavior may already be miscalibrated. Not scoped or built yet — next real investigation task.
