...
**5. No macro/weekly bias check exists for 4H/1H at all.** Confirmed via repo-wide grep: zero references to `macro_bias` or `weekly_200sma` anywhere in `_detect_4h_bos()`/`_detect_1h_bos()`. The 15M system's Macro Structural Architect checks this before ever approving a trade (per CLAUDE.md); 4H/1H has no equivalent, not even computed. This is not the same gap as #4 — it's a genuinely separate, currently-nonexistent check, not something that exists-but-isn't-gated.

**6. No KDE wall / airspace obstruction check exists for 4H/1H.** Confirmed via repo-wide grep: zero references to `kde_peak`/`gravity_kde`/`calculate_gravity_kde` in the BOS detectors. The 15M's Micro Liquidity Scavenger checks whether a heavy gravity wall sits between entry and target before calling the runway clear; 4H/1H can set a target directly into a wall and never know. Also genuinely separate from #4, currently nonexistent.
