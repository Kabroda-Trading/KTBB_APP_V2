...
def run_grid_simulation(config: GridConfig) -> Dict[str, Any]:
    ...
    # Pine Logic: totalWeight = (1 - sizeMult^maxBuys) / (1 - sizeMult)
    ...
    # We are deploying `deploy_pct` of the TOTAL capital
    ...
    # We loop to create 'max_buys' levels
    for i in range(config.max_buys):
        # A. Price Level Calculation
        # Pine Logic: nextTargetPrice = anchor - (stepSize * (tranchesTaken + 1))
        # Where stepSize = anchor * (gridPct / 100.0)
        
        step_size_usd = config.asset_price * (config.grid_step_pct / 100.0)
        # Level 1 is 1 step down, Level 2 is 2 steps down, etc.
        buy_price = config.asset_price - (step_size_usd * (i + 1))
        
        # B. Position Size Calculation
        # Pine Logic: tradeQty = base * math.pow(sizeMult, tranchesTaken)
