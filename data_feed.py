...
async def fetch_candles(symbol: str, timeframe: str, limit: int = 1000):
    try:
        ...
        if s == "BTC" or s == "BTCUSDT": symbol = "BTC/USDT"
        elif s == "ETH" or s == "ETHUSDT": symbol = "ETH/USDT"
...
# ---------------------------------------------------------
# 2. BATTLEBOX SUITE DATA FEED (Deep Fetch)
# ---------------------------------------------------------
async def get_inputs(symbol: str, date=None, session_tz="UTC"):
    try:
        # FETCH 2000 CANDLES (approx 20 days) for Research Lab backtesting
        daily, intraday = await asyncio.gather(
            fetch_candles(symbol, "1d", 100),
            fetch_candles(symbol, "15m", 2000) 
        )
