"""Edit the dashboard template - final version with all fixes."""
...
if old_live_tab_end not in content:
    ...
    with open('scratch/debug_marker.txt', 'w', encoding='utf-8') as f:
...
print("2. Added loadSessionEnergy() and loadHeartbeat() JS functions")

# ============================================================
# 3. Add calls in initDashboard
# ============================================================

old_init = """            loadLiveSystem().catch(e => console.error("loadLiveSystem failed", e)),
            loadParameters().catch(e => console.error("loadParameters failed", e)),"""

new_init = """            loadLiveSystem().catch(e => console.error("loadLiveSystem failed", e)),
...
if old_init not in content:
    print("ERROR: Could not find initDashboard loadLiveSystem call!")
    idx = content.find('loadLiveSystem().catch')
    with open('scratch/debug_init.txt', 'w', encoding='utf-8') as f:
        f.write(repr(content[idx-50:idx+200]))
else:
    content = content.replace(old_init, new_init, 1)
    print("3. Added loadSessionEnergy() and loadHeartbeat() to initDashboard")

# ============================================================
# 4. Add heartbeat interval
# ============================================================

# Find the initDashboard function and add setInterval for heartbeat
...
if old_interval not in content:
    ...
    with open('scratch/debug_interval.txt', 'w', encoding='utf-8') as f:
        f.write(repr(content[idx:idx+200]))
else:
    content = content.replace(old_interval, new_interval, 1)
    print("4. Added heartbeat interval")
