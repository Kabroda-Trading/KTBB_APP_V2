"""Edit the dashboard template - final version."""
...
# Find initDashboard and add the new load calls
old_init = """        loadAudits().catch(e => console.error("loadAudits failed", e)),
        loadLiveSystem().catch(e => console.error("loadLiveSystem failed", e)),"""

new_init = """        loadAudits().catch(e => console.error("loadAudits failed", e)),
        loadLiveSystem().catch(e => console.error("loadLiveSystem failed", e)),
...
if old_init not in content:
    ...
    print(f"loadLiveSystem at {idx}")
    with open('scratch/debug_init.txt', 'w', encoding='utf-8') as f:
        f.write(repr(content[idx-200:idx+200]))
else:
    content = content.replace(old_init, new_init, 1)
    print("4. Added loadSessionEnergy() and loadHeartbeat() to initDashboard")
