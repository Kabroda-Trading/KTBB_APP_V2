...
# ==============================================================================
# Purpose: Uses the CoinGlass API (Hobbyist 4H limits) to calculate Macro Fuel.
# Returns an OI/Funding multiplier to boost or suppress structural conviction.
# ==============================================================================
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            async with session.get(f"{BASE_URL}/futures/open-interest/history", params=oi_params, timeout=5) as oi_res:
                if oi_res.status == 200:
                    ...
                    if oi_data and len(oi_data) >= 2:
                        prev_oi, curr_oi = float(oi_data[-2][4]), float(oi_data[-1][4])
            ...
            async with session.get(f"{BASE_URL}/futures/funding-rate/history", params=fr_params, timeout=5) as fr_res:
                if fr_res.status == 200:
                    ...
                    if fr_data: telemetry["funding_rate"] = float(fr_data[-1][4])

        # CALCULATE FUEL MULTIPLIER
        # If OI is rising fast (> 1%), it boosts the macro wall's magnetic pull by 20%
