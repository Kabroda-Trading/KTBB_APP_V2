...
# ==============================================================================
# KABRODA BATTLEBOX PIPELINE — v9.0 (FUEL GAUGE INTEGRATION)
# ==============================================================================
# Purpose:
# - The single "Moment of Truth" for each session/day
# - Computes Multi-Timeframe Fuel (1H/4H EMAs, MACD, RSI) purely without dependencies.
...
def _build_fuel_gauge(raw_1h: List[Dict], raw_4h: List[Dict]) -> Dict:
    """Evaluates the structural fuel for the 1H and 4H timeframes."""
...
def _safe_placeholder_state(reason: str = "Waiting...") -> Dict[str, Any]:
    return {
        ...
        "execution": {"pause_state": "NONE", "resumption_state": "NONE", "gates_mode": "PREVIEW", "locked_at": None, "levels": {"failure": 0.0, "continuation": 0.0}}, 
...
def _compute_sse_packet(
    raw_5m: List[Dict], anchor_ts: int, macro_bias: str, micro_bias: str, fuel_gauge: Dict, tuning: Optional[Dict] = None, raw_daily: List[Dict] = None  
) -> Dict[str, Any]:
...
async def get_live_battlebox(symbol: str, session_mode: str = "AUTO", manual_id: Optional[str] = None, operator_flex: bool = False, tuning: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    if int(now_utc.timestamp()) < lock_end_ts:
        ...
        return {
            ...
            "battlebox": {
                ...
                "levels": {}, 
                "bias_model": {}, 
                "context": {"macro_bias": macro_bias, "micro_bias": micro_bias, "fuel_gauge": fuel_gauge}
            }
        }
    ...
    async with _CACHE_LOCK:
        if session_key not in _LOCKED_PACKETS:
            pkt = _compute_sse_packet(raw_5m, anchor_ts, macro_bias, micro_bias, fuel_gauge, tuning=tuning, raw_daily=raw_daily)
    ...
    return {
        ...
        "battlebox": {
            "war_map_context": _war_map_from_1h(raw_1h), 
