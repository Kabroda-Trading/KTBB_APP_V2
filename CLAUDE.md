...
3. **HTF carry** — ≥1 of {1H, 4H} trend (fresh 9/21 EMA read, `htf_fuel.py`) backs the side, **required for both fuel states as of 2026-09-10** (`decision_engine.py::_core_gate`, `htf_ok = aligned >= 1`; mirrored in `trade_plan.py::_stamp_tier_at_cross` so the pre-cross-anticipated path can't bypass it). From 2026-09-06 to 2026-09-10 this was waived entirely on the fuel-CONFLICTED path, on an 80-row sample showing HTF=0 CONFLICTED at +0.44R avg. The full corrected 5-year corpus did not hold that up: the `aligned=0` population is 154 trades, +0.08R avg, 51% win, 36% full-stop, and a walk-forward (fit 2021-2023 / verify 2024-2026) exposed it as overfit — fit window +20.3R / 66% win, held-out window −7.9R / 37% win. Mechanism: `aligned=0` winners average +0.90R vs +1.23R for `aligned≥1` (same loser size — without a higher-timeframe trend the runner has nothing to carry it). Andy-approved cut; full trail in `Kabroda AI Brain` repo `AGENT_LOG.md` 2026-09-10 + `LIVE_SYSTEM_STATE.md`.
...
**`PROMOTED_PUSH_FLOOR` = 1.8** (`decision_engine.py`, shipped 2026-09-10) — a *stricter* push-ratio bar that applies **only** when a `NO_PLAN` morning (no anticipated direction at lock) is re-evaluated on a later real cross and would otherwise promote to a live trade (`trade_plan.py::advance_no_plan`, the poll-`NO_PLAN` path). It does not touch the normal gate, WAITING plans, the radar, or lock-time evaluation. Evidence: the 5-year forensic (`Kabroda AI Brain` repo, `anticipate_replay.py` / `anticipate_replay_real.py`) found the ~120 "promoted" trades net +0.39R and are positive every year, but are bimodal on push strength — below ~1.8× baseline they are a coin flip that nets ~0R over the whole corpus; at 1.8×+ they are a real edge (~71 trades, +46R, ~74% win). "No direction at lock" is a mild negative that a strong push overrides. Drawdown-in-R is identical with or without this floor (it is not a risk cut); the effect is higher average R per promoted trade and a quieter surprise-`ARMED` email stream. Andy-approved; DeepSeek reproduced it including under the real BBWP classifiers — full trail `AGENT_LOG.md` 2026-09-10.
