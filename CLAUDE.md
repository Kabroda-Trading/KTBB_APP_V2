...
**`ledger_closing_engine.py`'s 30/70 rule is DEPRECATED as of 2026-09-07** — it predates the real audit (`CLEAN_REPORT.md`, Kabroda AI Brain repo) and was never updated to match it, including the later v2 rebuild below. **The actually implemented management rule (v2, current, 2026-09-11 T1 move + 2026-09-15 stop-never-moves ruling):** 50% at entry / 50% runner; T1 (`1.0×box`, moved from `0.618×` 2026-09-11) takes 50%, stop stays at the original level; the runner exits at T3 (`1.618×box`) or that same original stop — **it never moves, for any trade, not even at T2** (the old PREMIUM-only breakeven-at-T2 move was deleted outright 2026-09-15 once the tier split it depended on was retired — see "The Calibrated Gate" above for the full citation trail). This is what `executor_live_engine.py` (Domain 2, the real Bitunix executor) implements for real money — `CampaignLog`/`ledger_closing_engine.py` is the older, pre-audit shadow simulation (and now doubly stale: still 30/70, never updated to even the 2026-09-08 50/50 rule), kept running but not the source of truth anymore. Removal is a later cleanup pass once nothing references it (Andy's call, 2026-09-06) — not part of any build to date.
