...
def run_mas_analysis(
    ...
    # V2 block above or the Traveler injection block produced a
    # tradeable plan -- this must mean "the pipeline ran," not "a plan
    # was written," or a day where 6c's own bo/bd check is false would
    # falsely loop-retry forever (the same class of dedup bug already
    # hit once with MacroNarrativeLog). Non-blocking, same reasoning as
    # every other write in this function: a failure here must never
    # affect the SSOT writes already committed above.
    try:
