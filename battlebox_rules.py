# battlebox_rules.py
# ==============================================================================
# BATTLEBOX RULE LAYER v3.1 (AUDITED & ARMORED)
# ==============================================================================
from __future__ import annotations
from typing import Dict, List, Any, Optional

print(">>> LOADING BATTLEBOX RULES v3.1 (SAFE MODE) <<<") 
...
# --- FIX: ROBUST VOLUME CHECK ---
def check_volume_pressure(candles: List[Dict[str, Any]]) -> bool:
    if len(candles) < 21: return False
    try:
        # Safely extract volume, handling strings or missing keys
        def _get_vol(c):
            v = c.get("volume", 0)
            return float(v) if v is not None else 0.0

        curr = _get_vol(candles[-1])
        hist = candles[-21:-1]
        if not hist: return False

        avg = sum(_get_vol(c) for c in hist) / len(hist)
        if avg == 0: return True # Prevent weird logic if historical volume is 0
        return curr > (avg * 1.5)
    except Exception:
        # If any data is malformed, fail the check gracefully instead of crashing
        return False
