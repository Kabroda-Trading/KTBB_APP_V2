# market_radar.py
# ==============================================================================
# MARKET RADAR ENGINE (v2.2 - RICH METRICS + BIAS LOGIC)
# ==============================================================================
# 1. METRICS: Preserves strict Omega v16 text/color/percent formatting.
# 2. LOGIC: Adds Long/Short Bias detection to filter junk trades.
# 3. PREP: Pre-calculates Flight Plans for the Lock Target system.
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 1. METRIC CALCULATOR (PRESERVES YOUR RICH VISUALS)
# ------------------------------------------------------------------------------
def _calculate_rich_metrics(bps, r_mult, slope, struct_score):
    ...
    # D. HULL (10pts)
    h_val = 10 if struct_score > 0.5 else 0
    h_text = f"SOLID ({struct_score:.1f})" if struct_score > 0.5 else f"WEAK ({struct_score:.1f})"
    h_color = "CYAN" if struct_score > 0.5 else "RED"
...
# ------------------------------------------------------------------------------
# 2. ANALYSIS ENGINE (ADDS THE BRAINS)
# ------------------------------------------------------------------------------
def _analyze_session_kinetics(levels, price):
    ...
    # Calculate Rich Metrics for both
    ...
    # Protocol Status
    status = "DOGFIGHT"
    if bias == "GROUNDED" or final_score <= 40: status = "GROUNDED"
...
async def analyze_target(symbol, session_id="us_ny_futures"):
    # This powers the Lock Target page
    ...
    # Pass everything needed for the Omega Cockpit
    return {
