# research_lab.py
# ==============================================================================
# KABRODA RESEARCH LAB v14.0 (BATTLEBOX CORE & JSON REWRITE)
# ==============================================================================
# Updates:
# - REWRITE: Implements the clean "BattleBox Packet" JSON structure.
# - UI: Updated Strategy Names (S1=Expansion, S7=Continuation).
# - LOGIC: "Waiting For" story now references specific VAL/VAH levels.
# - FIX: Ensures Data Fetchers return Dictionaries to prevent crashes.
# ==============================================================================
...
# --- STORY ENGINE (NEW JSON FORMAT) ---
def _generate_story(phase, ledger, price, levels):
    # DYNAMIC LEVELS
    trigger_bull = levels.get("breakout_trigger", 0)
    trigger_bear = levels.get("breakdown_trigger", 0)
    ...
    if "CANDIDATE" in phase:
        if side == "Bull":
            return {
                "now": "Directional Bull candidate. Acceptance has printed, but structure is unconfirmed.",
                "wait": f"Waiting for a pullback retest into the {int(trigger_bull)} shelf that fails to reclaim value below {int(vah)} (VAH)."
            }
        else:
            return {
                "now": "Directional Bear candidate. Acceptance has printed, but structure is unconfirmed.",
                "wait": f"Waiting for a pullback retest into the {int(trigger_bear)} shelf that fails to reclaim value above {int(val)} (VAL)."
            }
...
def _build_safe_response(status="OK", msg="", session_info={}, levels={}, strategies=[], phase="INIT", ledger={}):
    # Matches the exact JSON structure requested
    return {
...
async def run_live_pulse(symbol: str, session_mode: str="AUTO", manual_id: str=None, risk_mode="fixed_margin", capital=1000, leverage=1) -> Dict[str, Any]:
    try:
        ...
        for code, func in auditors.items():
            try:
                ...
                if energy == "DEAD": status, msg = "OFF-HOURS", "Session exhausted."
                    ...
                    if code in ["S4", "S6"]: status = "BLOCKED"
                elif "CONFIRMED" in phase:
                    if "BULL" in phase:
                        if code in ["S1", "S7"]: status = "ACTIVE SIGNAL" if res['entry'] > 0 else "HUNTING"
                        elif code == "S2": status = "BLOCKED"
                    elif "BEAR" in phase:
                        if code in ["S2", "S7"]: status = "ACTIVE SIGNAL" if res['entry'] > 0 else "HUNTING"
                        elif code == "S1": status = "BLOCKED"
                    if code in ["S4", "S6"]: status = "BLOCKED"
                elif "FAILED" in phase and code == "S8": status = "ARMED"
                
                results.append({
                    "code": code,
                    "name": full_name,
                    "status": STATUS_MAPPING.get(status, status), # Mapped Display Status
                    "msg": msg,
                    "raw_status": status,
                    "color": "#00ff9d" if status in ["ACTIVE SIGNAL", "HUNTING"] else ("#ffcc00" if status == "ARMED" else "#444"),
                    "target": res['audit'].get('target', 0)
                })
            except: 
                results.append({"code": code, "name": code, "status": "Error", "color": "red"})

        # PACKET BUILD (Clean JSON)
        ...
        battlebox = {
            ...
            "strategies_summary": [
                {"code": r["code"], "name": r["name"], "status": r["status"], "msg": r["msg"]} 
                for r in results
            ]
        }
        ...
        return {
            ...
            "energy": energy,
            "levels": levels,
            "strategies": results, # For UI cards
            "battlebox": battlebox # For GPT
        }
